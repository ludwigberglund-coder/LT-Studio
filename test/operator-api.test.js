...
function legacyPasswordHash(password){
  const salt=Buffer.from('legacy-operator!','utf8');
...
test('operatorinloggning uppgraderar äldre lösenordshash först efter godkänd MFA',()=>withOperatorApi(async({runtime,base,operator})=>{
  const password='Operatorens starka testlosenord 2026!';
  const legacy=legacyPasswordHash(password);
  runtime.db.prepare('UPDATE platform_operators SET password_hash=? WHERE id=?').run(legacy,operator.id);
  assert.equal(Auth.needsPasswordRehash(Db.platformOperatorByUsername(runtime.db,'lt.operator').passwordHash),true);

  const badMfa=await fetch(base+'/api/operator/v1/auth/login',{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({username:'lt.operator',password,totp:'000000'})
  });
  assert.equal(badMfa.status,401);
  assert.equal(Db.platformOperatorByUsername(runtime.db,'lt.operator').passwordHash,legacy);

  const signed=await login(base);
  assert.equal(signed.response.status,200);
  const upgraded=Db.platformOperatorByUsername(runtime.db,'lt.operator').passwordHash;
  ...
  const audit=Db.platformOperatorAudit(runtime.db).find(event=>event.action==='OPERATOR_SESSION_LOGIN');
  assert.equal(audit.details.passwordHashUpgraded,true);
}));
