...
(async()=>{
  ...
  try{
    ...
    context.on('page',p=>{p.on('pageerror',e=>errors.push(e.message));p.on('dialog',d=>d.type()==='prompt'?d.accept('Automatiserat kreditprov'):d.accept());});
    ...
    const creditResponse=page.waitForResponse(r=>r.url().endsWith('/customer-invoices/'+f.issued.invoice.id+'/credit')&&r.request().method()==='POST');
    await page.getByRole('button',{name:'Kreditera hela fakturan',exact:true}).click();
    const credited=await creditResponse;assert.equal(credited.status(),201);
    await page.waitForFunction(()=>document.querySelector('#invoice-alert')?.textContent.includes('har krediterats'));
    const creditRows=Invoicing.listCustomerInvoices(f.db,f.a.id).filter(row=>Number(row.totalOre)<0);
    assert.equal(creditRows.length,1);assert.equal(Db.invoiceById(f.db,f.a.id,f.issued.invoice.id).remainingOre,0);
    await page.screenshot({path:path.join(out,'private-customer-credit.png'),fullPage:false});checks.push('Authorized full credit keeps original and creates source-linked credit invoice');
