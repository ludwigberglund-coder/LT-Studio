...
(async()=>{
  ...
  try{
    ...
    const countBeforeEnter=f.db.prepare('SELECT COUNT(*) AS n FROM invoices WHERE company_id=?').get(f.a.id).n;
    await page.locator('#invoice-form [name=customerNumber]').selectOption(f.customer.customerNumber);
    await page.locator('#invoice-form [name=invoiceDate]').fill('2026-09-21');
    await page.locator('#invoice-form [name=invoiceDate]').press('Enter');
    await page.waitForTimeout(150);
    assert.equal(f.db.prepare('SELECT COUNT(*) AS n FROM invoices WHERE company_id=?').get(f.a.id).n,countBeforeEnter);
    ...
    await page.getByRole('button',{name:'Till fakturalistan',exact:true}).click();
    ...
    const sourceInvoiceNumber=f.issued.invoice.invoiceNumber;
    await page.getByRole('button',{name:'Kreditera & kvitta',exact:true}).click();
    await page.waitForFunction(number=>document.querySelector('.invoice-toolbar h2')?.textContent?.includes('Kreditfaktura')&&document.body.textContent.includes('Krediterar faktura '+number),sourceInvoiceNumber);
    const sourceAfter=f.db.prepare('SELECT status,remaining_ore AS remainingOre FROM invoices WHERE company_id=? AND id=?').get(f.a.id,f.issued.invoice.id);
    assert.equal(sourceAfter.status,'Krediterad');
    assert.equal(sourceAfter.remainingOre,0);
    const credit=f.db.prepare("SELECT id,total_ore AS totalOre,status FROM invoices WHERE company_id=? AND total_ore<0 ORDER BY created_at DESC LIMIT 1").get(f.a.id);
    assert.ok(credit);
    assert.equal(credit.totalOre,-f.issued.invoice.totalOre);
    assert.equal(credit.status,'Kreditfaktura');
    checks.push('Unpaid customer invoice can be credited and offset to zero from the private invoice UI');
