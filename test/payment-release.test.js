...
function seed(){const db=Db.openDatabase(':memory:');SupplierAccounting.initializeSupplierAccounting(db);const company=Db.createCompany(db,{legalName:'Testbolag AB',displayName:'Testbolag',orgNumber:'559900-1010'});const hash=Auth.hashPassword('Sakert betalningstest 2026!');const preparer=Db.createUser(db,{username:'prep',displayName:'Förberedare',passwordHash:hash});const releaser=Db.createUser(db,{username:'release',displayName:'Frisläppare',passwordHash:hash});const accountant=Db.createUser(db,{username:'release-accountant',displayName:'Ekonom',passwordHash:hash});const supplier=Payables.createSupplier(db,{companyId:company.id,supplierNumber:'L-1',name:'Leverantören AB',bankgiro:'111-2222',defaultCostAccount:'4010'});const invoice=Payables.createSupplierInvoice(db,{companyId:company.id,supplierId:supplier.id,supplierInvoiceNumber:'F-1',invoiceDate:'2026-09-01',dueDate:'2026-09-16',totalOre:100000,vatOre:20000,registeredBy:preparer.id});Payables.storeDocument(db,{companyId:company.id,invoiceId:invoice.id,name:'faktura.pdf',bytes:Buffer.from('%PDF-1.4\n% test\n')});const coding=Domain.buildCoding({totalOre:100000,vatOre:20000});Payables.saveCoding(db,{companyId:company.id,invoiceId:invoice.id,lines:coding.lines});approveCurrent(db,company.id,invoice.id,releaser.id);SupplierAccounting.postSupplierInvoice(db,{companyId:company.id,invoiceId:invoice.id,actorId:accountant.id});const payment=Payables.preparePayment(db,{companyId:company.id,invoiceId:invoice.id,paymentDate:'2026-09-16',amountOre:100000,preparedBy:preparer.id});return{db,company,preparer,releaser,payment}}
