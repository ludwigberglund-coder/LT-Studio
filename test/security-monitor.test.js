...
test('säkerhetsmonitor failar stängt vid ofullständig eller trasig readiness',()=>{
  const db=Db.openDatabase(':memory:');
  try{
    const incomplete=SecurityMonitor.scanSecurityState(db,{readinessProvider:()=>({ok:false,checks:{}})});
    assert.equal(incomplete.status,'critical');
    assert.ok(incomplete.findings.some(item=>item.code==='READINESS_INCOMPLETE'));
...
test('bakgrundsmonitorn validerar skanningsintervallet',()=>{
  const db=Db.openDatabase(':memory:');
  try{
    assert.throws(
