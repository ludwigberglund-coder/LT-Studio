...
test('plattformsoverview listar företag och teknisk aktivitet utan affärsdetaljer',()=>{
...
test('plattformsoverview validerar databas och tidpunkt',()=>{
  assert.throws(()=>platformOverview(null),error=>error.code==='PLATFORM_OVERVIEW_DATABASE_REQUIRED');
  const db=Db.openDatabase(':memory:');
  try{assert.throws(()=>platformOverview(db,{nowMs:NaN}),error=>error.code==='PLATFORM_OVERVIEW_INVALID_TIME')}
  finally{db.close()}
});
