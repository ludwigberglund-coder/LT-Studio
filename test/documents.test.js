...
test('dokumentarkivet accepterar endast PDF-metadata',()=>{const {db,company,user}=seed();try{assert.throws(()=>Documents.createPending(db,{companyId:company.id,uploadedBy:user.id,title:'Bild',category:'receipt',fileName:'bild.png',mimeType:'image/png'}),e=>e.code==='UNSUPPORTED_DOCUMENT_TYPE'&&e.statusCode===415);assert.throws(()=>Documents.createPending(db,{companyId:company.id,uploadedBy:user.id,title:'Fel filändelse',category:'receipt',fileName:'underlag.jpg',mimeType:'application/pdf'}),e=>e.code==='PDF_EXTENSION_REQUIRED'&&e.statusCode===415);}finally{db.close()}});

test('dokumentarkivet blockerar PDF med aktivt innehåll',()=>{const {db,company,user}=seed();try{const doc=Documents.createPending(db,{companyId:company.id,uploadedBy:user.id,title:'Aktivt underlag',category:'other',fileName:'aktiv.pdf',mimeType:'application/pdf'});const active=Buffer.from('%PDF-1.4\n1 0 obj << /OpenAction 2 0 R /JavaScript (app.alert(1)) >> endobj\n%%EOF','latin1');assert.throws(()=>Documents.storeContent(db,{companyId:company.id,documentId:doc.id,bytes:active}),e=>e.code==='ACTIVE_PDF_CONTENT_NOT_ALLOWED'&&e.statusCode===415);}finally{db.close()}});
