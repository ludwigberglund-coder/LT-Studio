...
function seed(){
  ...
  Db.createInvoice(db,{companyId:company.id,customerId:customer.id,invoiceNumber:'1001',invoiceDate:'2026-09-05',postingDate:'2026-09-05',dueDate:'2026-10-05',totalOre:125000,vatOre:25000,remainingOre:125000});
  ...
  Payables.createSupplierInvoice(db,{companyId:company.id,supplierId:supplier.id,supplierInvoiceNumber:'S-1',invoiceDate:'2026-09-08',dueDate:'2026-10-08',totalOre:62500,vatOre:12500,registeredBy:user.id});
  ...
  Accounting.postEntry(db,{companyId:company.id,postingDate:'2026-09-05',description:'Kundfaktura 1001',sourceType:'invoice',sourceId:'1001',createdBy:user.id,series:'A',lines:[{account:'1510',debitOre:125000,creditOre:0},{account:'3010',debitOre:0,creditOre:100000},{account:'2611',debitOre:0,creditOre:25000}]});
  Accounting.postEntry(db,{companyId:company.id,postingDate:'2026-09-08',description:'Leverantörsfaktura S-1',sourceType:'supplier-invoice',sourceId:'S-1',createdBy:user.id,series:'A',lines:[{account:'4010',debitOre:50000,creditOre:0},{account:'2641',debitOre:12500,creditOre:0},{account:'2440',debitOre:0,creditOre:62500}]});
...
test('momsavstämning visar utgående och ingående moms men markeras ej deklarationsklar',()=>{const {db,company}=seed();try{const r=Reports.vatControl(db,company.id,{period:'2026-09'});assert.equal(r.outputVatOre,25000);assert.equal(r.inputVatOre,12500);assert.equal(r.netVatOre,12500);assert.equal(r.customerInvoiceCount,1);assert.equal(r.supplierInvoiceCount,1);assert.equal(r.declarationReady,false);assert.match(r.warning,/momsdeklaration/i);}finally{db.close()}});
