...
(async()=>{
  ...
  try{
    ...
    assert.equal(await paymentSearch.getAttribute('aria-expanded'),'true');
    assert.match(await dropdown.innerText(),/Leverantör Beta/);
    assert.equal(await page.locator('tbody tr').count(),1);
    assert.match(await page.locator('tbody tr').innerText(),/Leverantör Beta/);
    ...
    const responsePromise=page.waitForResponse(response=>response.url().includes('/api/v1/reports/payments-overview')&&response.url().includes('query=')&&!response.url().includes('query=Leverant')&&response.url().includes('account=1930')&&response.status()===200);
