...
async function start(filename){
  const runtime=createServer({databasePath:filename,secureCookies:false,authEncryptionKey:KEY});
...
async function login(base,{password=PASSWORD,totp=Auth.totpCode(MFA)}={}){
  return fetch(base+'/api/v1/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:'throttle.user',password,totp})});
}
