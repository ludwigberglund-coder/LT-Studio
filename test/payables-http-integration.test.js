...
test('riktiga HTTP-rutter genomför leverantörsfakturans bokföringsflöde utan dubbletter',async()=>{
  ...
  try{
    ...
    result=await request(base,`/api/v1/payables/invoices/${invoice.id}/coding`,accountantSession,{method:'PUT',body:{lines:changedLines}});assert.equal(result.response.status,200);const currentReview=result.data.invoice;assert.notEqual(currentReview.codingSha256,reviewed.codingSha256);
    result=await request(base,`/api/v1/payables/invoices/${invoice.id}/approve`,approverSession,{method:'POST',body:{expectedCodingSha256:reviewed.codingSha256,expectedDocumentSha256:reviewed.documentSha256}});assert.equal(result.response.status,409);assert.equal(result.data.code,'APPROVAL_STALE_CODING');
    result=await request(base,`/api/v1/payables/invoices/${invoice.id}`,approverSession);assert.equal(result.response.status,200);assert.equal(result.data.invoice.status,'coded');assert.equal(result.data.invoice.coding[0].account,'5460');
    assert.equal(Db.auditForCompany(db,company.id).filter(event=>event.action==='SUPPLIER_INVOICE_APPROVED').length,0);
    result=await request(base,`/api/v1/payables/invoices/${invoice.id}/approve`,approverSession,{method:'POST',body:{expectedCodingSha256:currentReview.codingSha256,expectedDocumentSha256:currentReview.documentSha256}});assert.equal(result.response.status,200);
    assert.equal(Db.auditForCompany(db,company.id).filter(event=>event.action==='SUPPLIER_INVOICE_APPROVED').length,1);
    ...
    result=await request(base,`/api/v1/payables/invoices/${invoice.id}/post`,accountantSession,{method:'POST',body:{}});assert.equal(result.response.status,200);const invoiceEntry=result.data.entry;assert.deepEqual(invoiceEntry.lines.map(line=>[line.account,line.debitOre,line.creditOre]),[['5460',100000,0],['2641',25000,0],['2440',0,125000]]);
    ...
    result=await request(base,`/api/v1/payables/invoices/${invoice.id}/correct-dates`,accountantSession,{method:'POST',body:{requestId:'http-supplier-date-correction-0001',invoiceDate:'2026-09-09',dueDate:'2026-09-17',reason:'Fakturadatum rättat mot original-PDF'}});assert.equal(result.response.status,200);assert.equal(result.data.duplicate,false);assert.equal(result.data.invoice.invoiceDate,'2026-09-09');assert.equal(result.data.invoice.openAmountOre,125000);assert.equal(result.data.reversal.postingDate,'2026-09-08');assert.equal(result.data.replacement.postingDate,'2026-09-09');
