    ...
    for(const viewport of viewports){
      ...
      for(const surface of surfaces){
        ...
        if(surface.id==='portal-index'){
          ...
          await search.fill('K-10');
          await page.waitForTimeout(80);
          const suggestionBox=page.locator('#receivable-search-results');
          assert.equal(await suggestionBox.isVisible(),true,`customer number typing must open the suggestion list on ${viewport.id}`);
          assert.ok(await suggestionBox.locator('.receivable-search-option[data-kind="customer"]').count()>=4,`customer number search must list all matching customers on ${viewport.id}`);
          assert.match(await suggestionBox.innerText(),/K-1002/,`customer number K-1002 must appear in suggestions on ${viewport.id}`);
          const inputRect=await search.boundingBox(),listRect=await suggestionBox.boundingBox();
          assert.ok(inputRect&&listRect&&listRect.y>=inputRect.y+inputRect.height-2,`suggestion list must open directly below the search input on ${viewport.id}`);
          await search.fill('31000');
          await page.waitForTimeout(80);
          assert.ok(await suggestionBox.locator('.receivable-search-option[data-kind="invoice"]').count()>=4,`invoice prefix search must list all matching invoices on ${viewport.id}`);
          assert.match(await suggestionBox.innerText(),/Faktura 310002/,`invoice 310002 must appear while typing on ${viewport.id}`);
          await search.fill('Nord');
          await page.waitForTimeout(80);
          assert.match(await suggestionBox.innerText(),/Nordic Office Göteborg AB/,`customer name prefix must appear while typing on ${viewport.id}`);
          await suggestionBox.locator('.receivable-search-option[data-kind="customer"]').filter({hasText:'Nordic Office Göteborg AB'}).click();
          await page.waitForTimeout(80);
          assert.equal(await suggestionBox.isVisible(),false,`selecting a suggestion must close the list on ${viewport.id}`);
          assert.equal(await page.locator('.invoice-row:visible').count(),1,`selecting the customer suggestion must filter to that customer's receivables on ${viewport.id}`);
          await search.fill('');
          await search.fill('3');
