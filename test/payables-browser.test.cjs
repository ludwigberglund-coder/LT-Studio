...
(async()=>{
  ...
  try{
    ...
    await reviewedDebit.fill('999,00');assert.equal(await page.getByRole('button',{name:'Spara kontering'}).isDisabled(),true);assert.equal(await page.getByRole('button',{name:'Attestera faktura'}).isDisabled(),true);const balanceBox=page.locator('[data-coding-balance]'),balanceStatus=page.locator('[data-balance-status-unbalanced]');assert.equal(await balanceBox.getAttribute('data-balance-state'),'unbalanced');assert.equal((await balanceStatus.textContent())?.trim(),'Måste balansera före attest');assert.equal(await balanceStatus.isVisible(),true);await reviewedDebit.fill('1000,00');assert.equal(await page.getByRole('button',{name:'Attestera faktura'}).isDisabled(),false);
    await reviewedAccount.fill('5460');
    await page.getByRole('button',{name:'Attestera faktura'}).click();await page.getByText(/osparade ändringar/i).waitFor({timeout:10000});
