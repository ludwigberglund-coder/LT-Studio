...
(async()=>{
  ...
  try{
    ...
    await reviewedDebit.fill('999,00');const saveCoding=page.getByRole('button',{name:'Spara kontering'}),approveButton=page.getByRole('button',{name:'Attestera faktura'}),balanceBox=page.locator('[data-coding-balance]'),balanceStatus=page.locator('[data-balance-status-unbalanced]');assert.equal(await saveCoding.getAttribute('aria-disabled'),'true');assert.equal(await approveButton.getAttribute('aria-disabled'),'true');assert.equal(await balanceBox.getAttribute('data-balance-state'),'unbalanced');assert.equal((await balanceStatus.textContent())?.trim(),'Måste balansera före attest');assert.equal(await balanceStatus.isVisible(),true);await approveButton.click();await page.locator('.payables-action-toast.is-error').getByText(/Konteringen måste balansera före attest/).waitFor({timeout:10000});assert.equal(await reviewedDebit.inputValue(),'999,00');await reviewedDebit.fill('1000,00');assert.equal(await approveButton.getAttribute('aria-disabled'),'false');
    ...
    await page.getByRole('button',{name:'Attestera faktura'}).click();await page.locator('.payables-action-toast').getByText(/Fakturan är attesterad/).waitFor({timeout:10000});await page.getByText('Attesterad',{exact:true}).waitFor();await page.getByRole('button',{name:'Bokför leverantörsskuld'}).waitFor({timeout:10000});
    ...
    db.prepare('UPDATE supplier_invoices SET vat_treatment=NULL WHERE company_id=? AND id=?').run(company.id,invoice.id);await page.reload({waitUntil:'networkidle'});await page.getByText('Äldre bokförd post · momsmarkering saknas',{exact:true}).waitFor({timeout:10000});
