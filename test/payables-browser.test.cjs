...
(async()=>{
  ...
  try{
    ...
    await reviewedDebit.fill('999,00');assert.equal(await page.getByRole('button',{name:'Spara kontering'}).isDisabled(),true);assert.equal(await page.getByRole('button',{name:'Attestera faktura'}).isDisabled(),true);await page.getByText('Måste balansera före attest',{exact:true}).waitFor();await reviewedDebit.fill('1000,00');assert.equal(await page.getByRole('button',{name:'Attestera faktura'}).isDisabled(),false);
