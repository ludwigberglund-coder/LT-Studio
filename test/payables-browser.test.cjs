...
(async()=>{
  ...
  try{
    ...
    await reviewedDebit.fill('999,00');assert.equal(await page.getByRole('button',{name:'Spara kontering'}).isDisabled(),true);assert.equal(await page.getByRole('button',{name:'Attestera faktura'}).isDisabled(),true);const balanceStatus=page.locator('.coding-balance.unbalanced [data-balance-status-unbalanced]');assert.equal((await balanceStatus.textContent())?.trim(),'Måste balansera före attest');assert.equal(await balanceStatus.isVisible(),true);await reviewedDebit.fill('1000,00');assert.equal(await page.getByRole('button',{name:'Attestera faktura'}).isDisabled(),false);
