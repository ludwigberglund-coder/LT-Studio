...
(async()=>{
  ...
  try{
    ...
    await page.getByRole('button',{name:'Attestera faktura'}).click();await page.locator('.payables-action-toast').getByText(/Fakturan är attesterad/).waitFor({timeout:10000});await page.getByText('Attesterad – skuld ej bokförd',{exact:true}).waitFor();await page.getByRole('button',{name:'Bokför leverantörsskuld'}).waitFor({timeout:10000});
