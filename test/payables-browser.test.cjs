...
(async()=>{
  ...
  try{
    ...
    await page.getByText('BKS-771',{exact:true}).first().click();await page.getByText('Svensk faktura · full avdragsrätt',{exact:true}).waitFor();await page.getByRole('button',{name:'Hämta konteringsförslag'}).click();await page.getByRole('button',{name:'Spara kontering'}).click();
    const queueSearch=page.locator('.queue-search');await queueSearch.click();await page.keyboard.type('BKS-771');assert.equal(await queueSearch.inputValue(),'BKS-771');assert.equal(await queueSearch.evaluate(node=>document.activeElement===node),true);assert.equal(await page.locator('.queue-row').count(),1);assert.match(await page.locator('.queue-row').innerText(),/BKS-771/);await queueSearch.fill('');

    await setSession(approverSession);await page.getByText('BKS-771',{exact:true}).first().click();
    const reviewedAccount=page.locator('[data-line="0"][data-field="account"]');assert.equal(await reviewedAccount.inputValue(),'4010');await reviewedAccount.fill('5460');
