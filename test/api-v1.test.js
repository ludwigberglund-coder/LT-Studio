...
test('lyckad MFA-inloggning uppgraderar en äldre lösenordshash utan lösenordsbyte', async () => withApi(async ({base,password,db,user,co1}) => {
  const legacy=legacyPasswordHash(password);
  db.prepare('UPDATE users SET password_hash=? WHERE id=?').run(legacy,user.id);
  assert.equal(Auth.needsPasswordRehash(Db.userByUsername(db,user.username).passwordHash),true);

  const badMfa=await fetch(`${base}/api/v1/auth/login`,{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({username:user.username,password,totp:'000000'})
  });
  assert.equal(badMfa.status,401);
  assert.equal(Db.userByUsername(db,user.username).passwordHash,legacy);

  const signed=await login(base,password);
  assert.equal(signed.response.status,200);
  const upgraded=Db.userByUsername(db,user.username).passwordHash;
  assert.notEqual(upgraded,legacy);
  assert.equal(Auth.verifyPassword(password,upgraded),true);
  assert.equal(Auth.needsPasswordRehash(upgraded),false);
  const loginAudit=Db.auditForCompany(db,co1.id).find(event=>event.action==='SESSION_LOGIN');
...
test('fel lösenord och okänt användarnamn ger samma externa svar', async () => withApi(async ({base}) => {
  const request=username=>fetch(`${base}/api/v1/auth/login`,{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({username,password:'fel fel fel fel fel fel'})
  });
  const existing=await request('sara.test'),unknown=await request('saknas.test');
  const existingBody=await existing.json(),unknownBody=await unknown.json();
  assert.equal(existing.status,401);
  assert.equal(unknown.status,401);
  assert.equal(existingBody.code,'INVALID_CREDENTIALS');
  assert.equal(unknownBody.code,'INVALID_CREDENTIALS');
  assert.equal(existingBody.error,unknownBody.error);
}));
