...
async function withApi(callback) {
  ...
  const user=Db.createUser(db,{username:'sara.test',displayName:'Sara Test',passwordHash:Auth.hashPassword(password)});
...
async function login(base,password) {
  const response=await fetch(`${base}/api/v1/auth/login`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:'sara.test',password})});
  const body=await response.json();
  const cookie=String(response.headers.get('set-cookie') || '').split(';')[0];
  return {response,body,cookie};
}

test('login skapar serverlagrad session och kundreskontran kräver session', async () => withApi(async ({base,password,inv1}) => {
