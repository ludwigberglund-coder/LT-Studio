-- Extend source-controlled financial batches to supplier liabilities and payroll.
-- Both flows remain outside the ledger/reskontra until the batch is approved.
...
begin
  ...
  if p_activation_type not in ('customer-invoice','supplier-liability','payroll-run') then raise exception 'UNSUPPORTED_SOURCE_BATCH_ACTIVATION'; end if;
...
begin
  ...
  loop
    ...
    if v_tx.activation_type is not null then
      ...
      if v_tx.activation_type='customer-invoice' then
      ...
      elsif v_tx.activation_type='supplier-liability' then
        if v_tx.source_type<>'supplier-invoice' or v_series<>'A' or coalesce(v_tx.activation_payload->>'invoiceId',v_tx.source_id)<>v_tx.source_id then
          raise exception 'SUPPLIER_LIABILITY_BATCH_INTEGRITY_ERROR';
        end if;
        ...
        if v_tx.source_type<>'payroll-run' or v_series<>'L' or coalesce(v_tx.activation_payload->>'payrollRunId',v_tx.source_id)<>v_tx.source_id then
          raise exception 'PAYROLL_BATCH_INTEGRITY_ERROR';
        end if;
        ...
        where r.company_id=p_company_id
          ...
          and r.accounting_entry_id is null;
...
declare
  ...
  v_coding jsonb; v_existing_entry text; v_period text; v_line jsonb; v_debit numeric:=0; v_credit numeric:=0;
  v_batch_id text; v_batch_number integer; v_existing_batch_id text; v_existing_batch_number integer; v_existing_batch_status text;
begin
  if v_uid is null then raise exception 'AUTH_REQUIRED'; end if;
  ...
  if not exists(
    select 1 from public.company_memberships m
    where m.company_id=v_company_id and m.auth_user_id=v_uid and m.role in ('admin','accountant')
  ) then raise exception 'ACCESS_DENIED'; end if;
  ...
  if v_existing_entry is not null then
    ...
    where j.company_id=v_company_id and j.id=v_existing_entry;
    return;
  end if;

  select b.id,b.batch_number,b.status
    into v_existing_batch_id,v_existing_batch_number,v_existing_batch_status
  ...
  where t.company_id=v_company_id
    and t.source_type='supplier-invoice'
    and t.source_id=p_invoice_id
    and t.activation_type='supplier-liability'
  order by b.created_at desc
  limit 1;
  if found then
    return query select null::text,'A'::text,lpad(v_existing_batch_number::text,5,'0'),v_posting_date,'pending-batch'::text;
    return;
  end if;
  ...
  v_period:=to_char(v_posting_date,'YYYY-MM');
  if exists(
    select 1 from public.accounting_periods p
    where p.company_id=v_company_id and p.period=v_period and p.status='locked'
  ) then raise exception 'PERIOD_LOCKED'; end if;
  ...
  from public.stage_source_financial_batch(
    v_company_id,
    'Leverantörsfaktura '||v_invoice_number,
    'A',
    v_posting_date,
    left('Leverantörsfaktura '||v_invoice_number,240),
    'supplier-invoice',
    p_invoice_id,
    v_coding,
    'supplier-liability',
    jsonb_build_object('invoiceId',p_invoice_id,'supplierInvoiceNumber',v_invoice_number)
  ) s;

  return query select null::text,'A'::text,lpad(v_batch_number::text,5,'0'),v_posting_date,'pending-batch'::text;
end;
$$;
...
as $$
declare
  v_uid uuid:=auth.uid();v_run public.payroll_runs%rowtype;v_period text;v_line jsonb;v_debit numeric:=0;v_credit numeric:=0;
  v_batch_id text;v_batch_number integer;v_existing_batch_number integer;
begin
  ...
  select b.batch_number into v_existing_batch_number
  from public.financial_batch_transactions t
  join public.financial_batches b on b.company_id=t.company_id and b.id=t.batch_id
  where t.company_id=p_company_id
    and t.source_type='payroll-run'
    and t.source_id=v_run.id
    and t.activation_type='payroll-run'
  order by b.created_at desc
  limit 1;
  if found then
    return query select v_run.id,lpad(v_existing_batch_number::text,5,'0'),'pending-batch'::text,true;
    return;
  end if;
  ...
  if encode(extensions.digest(convert_to(v_run.lines_json::text,'UTF8'),'sha256'),'hex')<>v_run.journal_sha256 then
    raise exception 'PAYROLL_POSTING_INTEGRITY_ERROR';
  end if;
  v_period:=to_char(v_run.pay_date,'YYYY-MM');
  if exists(
    select 1 from public.accounting_periods ap
  ...
  from public.stage_source_financial_batch(
    ...
    v_run.lines_json,
    'payroll-run',
    jsonb_build_object('payrollRunId',v_run.id,'period',v_run.period)
  ) s;

  return query select v_run.id,lpad(v_batch_number::text,5,'0'),'pending-batch'::text,false;
end;
$$;
