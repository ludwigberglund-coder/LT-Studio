...
function validateOperations(value){
  ...
  if(value.approvedForPilot!==true)fail.push('approvedForPilot måste vara true efter ett uttryckligt pilotbeslut.');
  const approvedAt=String(value.approvedAt||'').trim();
  if(!/^\d{4}-\d{2}-\d{2}$/.test(approvedAt))fail.push('approvedAt måste vara ett datum på formen ÅÅÅÅ-MM-DD.');
  return{ok:fail.length===0,fail};
}
function validateOperationsFile(filename){
