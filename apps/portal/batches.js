...
function render(){
 ...
 if(selected){
   const editable=canEdit()&&['draft','rejected'].includes(selected.status);
   ...
   if(ready&&canEdit())controls.push('<button class="button ghost" data-action="reopen">Återöppna</button>');
   if(ready&&canApprove())controls.push('<button class="button ghost" data-action="reject">Avvisa</button>','<button class="button" data-action="approve">Godkänn bunt</button>');
   editor='<div class="batch-form-row"><label>Namn<input id="batch-title" value="'+esc(selected.title)+'" '+(!editable?'disabled':'')+'></label><label>Externt kontrollbelopp (kr)<input id="batch-external" inputmode="decimal" value="'+esc(selected.external_total_ore==null?'':selected.external_total_ore/100)+'" '+(!editable?'disabled':'')+'></label></div><div class="batch-summary"><span class="summary-pill">Bunt #'+String(selected.batch_number).padStart(5,'0')+'</span><span class="summary-pill">'+selected.transaction_count+' transaktioner</span><span class="summary-pill">Debet '+kr(selected.total_debit_ore)+'</span><span class="summary-pill">Kredit '+kr(selected.total_credit_ore)+'</span><span class="summary-pill '+(selected.control_state==='balanced'?'ok':'err')+'">'+(selected.control_state==='balanced'?'Balanserad':'Kontroll krävs')+'</span>'+badge(selected.status)+'</div><div id="transactions">'+txs.map(txTemplate).join('')+'</div>'+(editable?'<button class="button ghost" data-action="add-tx">+ Lägg till transaktion</button>':'')+'<div class="batch-actions">'+controls.join('')+'</div>'+(ready&&canApprove()?'<p class="batch-meta">Godkännaren måste vara en annan person än den som skapade eller senast kontrollerade bunten.</p>':'')+'<details><summary>Revisionsspår</summary><div class="batch-meta">'+(events.length?events.map(e=>esc(new Date(e.created_at).toLocaleString('sv-SE'))+' · '+esc(e.event_type)).join('<br>'):'Inga händelser ännu.')+'</div></details>';
 }
 app.innerHTML='<main class="batch-shell"><div class="batch-head"><div><h1>Buntar</h1><p>Ekonomiskt granskningslager. Inget påverkar huvudboken före godkännande.</p></div><div class="batch-actions"><input class="batch-search" data-search placeholder="Sök buntnummer" value="'+esc(searchQuery)+'">'+(canEdit()?'<button class="button" data-action="new">+ Ny bunt</button>':'')+'</div></div><div class="batch-layout"><aside class="batch-list">'+(list||'<div class="empty">Inga buntar hittades.</div>')+'</aside><article class="batch-editor">'+editor+'</article></div></main>';
}
async function load(){
