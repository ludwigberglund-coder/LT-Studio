...
async function supabaseContext(){
  supabaseCtx=supabaseCtx?.authenticated?supabaseCtx:await window.LTSupabaseUat.context();
  if(!supabaseCtx?.authenticated||!supabaseCtx.company){location.href='./index.html';throw new Error('Ingen aktiv Supabase-session.');}
  if(supabaseCtx.membership?.role!=='admin')throw new Error('Endast företagsadmin kan hantera webbplatsens innehåll.');
  return supabaseCtx;
}
async function loadSupabaseCms(){
  ...
  const [states,history]=await Promise.all([
    window.LTSupabase.from('website_cms_state',ctx.accessToken).select('*',companyFilter),
    window.LTSupabase.from('website_cms_revisions',ctx.accessToken).select('*',companyFilter+'&order=version.desc')
  ]);
  const row=states?.[0];
  if(!row)throw new Error('Webbplatsens Supabase-state saknas för företaget.');
  cms={
    draft:{
      revision:Number(row.draft_revision||1),
      site:clone(row.draft_site),
      company:clone(row.draft_company),
      updatedBy:row.draft_updated_by||null,
      updatedAt:row.draft_updated_at
    },
    published:{
      site:clone(row.published_site),
      company:clone(row.published_company),
...
async function saveDraft(){
  ...
  if(isSupabase){
    ...
    await window.LTSupabase.rpc('save_website_cms_draft',{
      p_company_id:ctx.company.id,
      p_expected_revision:cms.draft.revision,
      p_site:draft.site,
      p_company:draft.company
    },ctx.accessToken);
    await loadSupabaseCms();
    dirty=false;message='Utkastet är sparat i Supabase.';errorMessage='';render();return;
  }
...
function setPreview(){
  ...
  const draft=isSupabase?{site:clone(cms.draft.site),company:clone(cms.draft.company)}:readDraft();
...
async function previewDraft(){
  ...
  tab.opener=null;
  try{
    if(isDemo)tab.location.href=setPreview();
    else if(isSupabase){await saveDraft();tab.location.href=setPreview();}
    else{await saveDraft();tab.location.href='/website-preview/';}
  }
  catch(error){tab.close();throw error;}
}
async function publish(){
  if(isDemo){
    demoSave();const stamp=new Date().toISOString(),version=Number(cms.published.version||0)+1;
    snapshots[version]={site:clone(cms.draft.site),company:clone(cms.draft.company)};
    cms.published={site:clone(cms.draft.site),company:clone(cms.draft.company),version,publishedBy:'demo-user',publishedAt:stamp};
    revisions.unshift({id:`demo-${version}`,version,publishedBy:'demo-user',publishedAt:stamp});
    saveDemoState();message=`Demoversion ${version} publicerades lokalt. Ingen GitHub-fil eller verklig webbplats ändrades.`;render();return;
  }
  await saveDraft();
  if(isSupabase){
    const ctx=await supabaseContext();
    await window.LTSupabase.rpc('publish_website_cms',{
      p_company_id:ctx.company.id,
      p_expected_revision:cms.draft.revision,
      p_expected_published_version:cms.published.version
    },ctx.accessToken);
    await loadSupabaseCms();
    message=`CMS-version ${cms.published.version} är publicerad i Supabase.`;errorMessage='';render();return;
  }
  const data=await api('/website/cms/publish',{method:'POST',body:{expectedRevision:cms.draft.revision,expectedPublishedVersion:cms.published.version}});
  cms=data.state;revisions=data.revisions||[];message=data.message;errorMessage='';render();
}
async function restore(version){
  if(isDemo){
    const snapshot=snapshots[version];if(!snapshot)throw new Error('Den valda demoversionen saknar sparat innehåll.');
    cms.draft={...cms.draft,site:clone(snapshot.site),company:clone(snapshot.company),updatedAt:new Date().toISOString(),updatedBy:'demo-user'};
    saveDemoState();message=`Version ${version} återställdes som utkast. Den publicerade demoversionen ändrades inte.`;errorMessage='';render();return;
  }
  if(isSupabase){
    const ctx=await supabaseContext();
    await window.LTSupabase.rpc('restore_website_cms_revision',{
      p_company_id:ctx.company.id,
      p_version:Number(version),
      p_expected_revision:cms.draft.revision
    },ctx.accessToken);
    await loadSupabaseCms();
    message=`Version ${version} återställdes som Supabase-utkast.`;errorMessage='';render();return;
  }
  const data=await api(`/website/cms/revisions/${encodeURIComponent(version)}/restore`,{method:'POST',body:{expectedRevision:cms.draft.revision}});
  cms=data.state;message=data.message;errorMessage='';render();
}
async function boot(){
  try{
    if(isDemo){
      ...
      if(saved?.cms){cms=saved.cms;revisions=saved.revisions||[];snapshots=saved.snapshots||{}}
      else{
        const [site,company]=await Promise.all([loadJson('../content/site.json'),loadJson('../content/company.json')]);
        const stamp=new Date().toISOString();
        cms={draft:{site:clone(site),company:clone(company),updatedAt:stamp,updatedBy:'demo-user'},published:{site:clone(site),company:clone(company),version:0,publishedBy:null,publishedAt:null}};
        revisions=[];snapshots={};
      }
      render();return;
    }
    if(isSupabase){await loadSupabaseCms();render();return;}
    const state=await api('/session');if(!state.authenticated){location.href='./index.html';return}
    session=state;const data=await api('/website/cms');cms=data.state;revisions=data.revisions||[];render();
  }catch(error){app.innerHTML=`<main class="boot"><strong>Kunde inte ladda webbplats-CMS</strong><span>${esc(error.message)}</span></main>`;}
}
async function reloadDraft(){
  if(isDemo){await boot();dirty=false;return;}
  if(isSupabase){
    await loadSupabaseCms();dirty=false;
    message='Senaste Supabase-utkastet hämtades.';errorMessage='';return;
  }
