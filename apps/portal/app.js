...
const demoInvoices=[
  {id:'demo-i1',kind:'customer',customerNumber:'K-1001',customerName:'Västra Hamnen Logistik AB',invoiceNumber:'310001',ocr:'310001',invoiceDate:'2026-08-18',postingDate:'2026-08-18',dueDate:'2026-09-17',totalOre:420000,remainingOre:420000,vatOre:45000,status:'Förfallen',paymentMethod:'Bankgiro',paymentAccount:'BG 123-4567',invoiceAccount:'1510',batchNumber:'1028',journalNumber:'A28',customerType:'business',reminderFeeAgreed:true,commentCount:1,transactions:[],reminders:[{id:'demo-r1',sentAt:'2026-09-19T09:00:00.000Z',kind:'payment-reminder'}]},
...
function matchingCustomerIds(){
  ...
  for(const customer of receivableCustomers){
    const identity=[customer.customerName,customer.customerNumber,customer.orgNumber].map(normalizeSearch);
    const invoiceMatch=invoicesForCustomer(customer.customerId).some(invoice=>[invoice.invoiceNumber,invoice.ocr].map(normalizeSearch).some(value=>value.includes(query)));
    if(identity.some(value=>value.includes(query))||invoiceMatch)matches.add(String(customer.customerId));
  }
  return matches;
}
...
function table(){
  ...
  const customers=visibleReceivableCustomers();
  const bodies=customers.map(customer=>{
    const customerInvoices=invoicesForCustomer(customer.customerId).map(withDemoState);
    const customerTotal=Number(customer.remainingOre||0);
    const customerHeader=`<tr class="customer-summary-row" data-customer-id="${escapeHtml(customer.customerId)}"><td class="customer-cell"><div class="customer-identity"><span><b>${escapeHtml(customer.customerNumber||'—')}</b><strong>${escapeHtml(customer.customerName||'Okänd kund')}</strong>${customer.orgNumber?`<small>Org.nr ${escapeHtml(customer.orgNumber)}</small>`:''}</span><span class="customer-rest-badge"><small>Restbelopp</small><b>${ore(customerTotal)}</b></span></div></td>${columns.map(column=>column.id==='remainingOre'?`<td class="money customer-total-rest"><b>${ore(customerTotal)}</b></td>`:'<td class="customer-summary-empty">—</td>').join('')}</tr>`;
    const invoiceRows=customerInvoices.map(invoice=>{
      const base=R.receivableRow(invoice);
      const comments=invoice.commentCount||0;
      const invoiceRow=`<tr class="invoice-row" data-invoice-id="${escapeHtml(invoice.id)}"><td class="customer-cell invoice-customer-cell"><span>↳ Faktura ${escapeHtml(invoice.invoiceNumber||'—')}</span>${comments?`<span class="comment-badge">💬 ${comments}</span>`:''}</td>${columns.map(c=>cell(c,base)).join('')}</tr>`;
    ...
    return customerHeader+invoiceRows;
  }).join('');
