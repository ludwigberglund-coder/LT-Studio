...
async function refresh(){
  if(operatorRefreshing)return;
  operatorRefreshing=true;errorMessage='';uiNotice='';render();
  try{
    await loadData();
    if(selectedCompany)selectedCompany=await api('/companies/'+encodeURIComponent(selectedCompany.company.id));
    uiNotice=`Adminvyn är uppdaterad ${new Intl.DateTimeFormat('sv-SE',{hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(new Date())}.`;
  }catch(error){errorMessage=error.message}
  finally{operatorRefreshing=false;render()}
}
...
document.addEventListener('submit',async event=>{
  ...
  if(event.target.id==='add-user-form'){
    ...
    try{const created=await mutate('/companies/'+encodeURIComponent(selectedCompany.company.id)+'/users',{method:'POST',body:JSON.stringify(data)});await reloadSelectedCompanyOverview();uiNotice=created.linkedExisting?(useSupabase?'Befintligt Supabase-konto kopplades till företaget.':'Befintligt konto kopplades till företaget.'):(useSupabase?'Användaren skapades i Supabase Auth.':'Användaren skapades i den privata operatörsmiljön.');render();const box=document.getElementById('mfa-result');if(box&&created.mfaEnrollmentRequired)box.innerHTML=`<div class="success-box"><strong>MFA registreras av användaren</strong><p>Användaren ska logga in och registrera sin TOTP-faktor i Supabase Auth. LT Studio visar eller lagrar inte MFA-hemligheten i adminportalen.</p></div>`}catch(error){errorMessage=error.message;render()}return;
  }
