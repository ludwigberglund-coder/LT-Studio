...
let session=null,overview=null,readiness=null,security=null,securityMonitor=null,securityPollTimer=null,errorMessage='',view='overview',selectedCompany=null,modal=null,uiNotice='',companyQuery='',companyStatus='all',companySort='name',securitySeverity='all',securityPeriod='24h';
...
function securityFindings(){
  ...
  if(!findings.length)return '<div class="monitor-clear"><span class="clear-check" aria-hidden="true">✓</span><div><strong>Inga aktiva säkerhetsflaggor</strong><p>Senaste skanningen hittade inga regler som kräver åtgärd.</p></div></div>';
  return `<div class="monitor-finding-list">${findings.map(item=>`<article class="monitor-finding ${esc(item.severity)}"><div class="finding-top"><span class="status-pill"><span class="dot ${esc(item.severity)}"></span>${esc(({critical:'Kritisk',warning:'Varning',info:'Information'})[item.severity]||item.severity)}</span><span class="finding-category">${esc(item.category)}</span></div><h3>${esc(item.title)}</h3><p>${esc(item.message)}</p></article>`).join('')}</div>`;
}
...
function securityView(){
  ...
  const critical=events.filter(event=>event.severity==='critical').length,warning=events.filter(event=>event.severity==='warning').length,monitorTone=monitor.status==='critical'?'critical':monitor.status==='attention'?'warning':'ok';
  shell(`<section class="security-hero security-hero-live active-security-hero"><div><span class="eyebrow">AKTIV SÄKERHETSPORTAL</span><h2>Verkliga kontroller. Automatisk riskjakt.</h2><p>Readiness, verifieringsbevis och säkerhetshändelser kompletteras nu av en serverbaserad monitor som söker efter risker även när sidan inte är öppen.</p><div class="monitor-live"><span class="live-dot ${monitorTone}"></span><strong>Aktiv övervakning</strong><span>Senast skannad ${dateTime(monitor.checkedAt)} · cirka var ${num(monitor.scanIntervalSeconds||30)} sekund</span></div></div><div class="security-snapshot"><div><span>Tekniska kontroller</span><strong>${okCount}/${known.length||entries.length}</strong><small>rapporterar OK</small></div><div><span>MFA-täckning</span><strong>${mfaPct}%</strong><small>${totals.mfaProtectedUsers||0} av ${totals.activeUsers||0} aktiva användare</small></div><div><span>Aktiva flaggor</span><strong class="${monitorTone}">${num(counts.total)}</strong><small>${num(counts.critical)} kritiska · ${num(counts.warning)} varningar</small></div></div></section>
  <section class="status-grid"><article class="metric"><span>Readiness</span><strong class="${readiness?.ok?'ok':'critical'}">${readiness?.ok?'OK':'Åtgärd krävs'}</strong><small>samlad teknisk gate</small></article><article class="metric"><span>Aktiva flaggor</span><strong class="${monitorTone}">${num(counts.total)}</strong><small>från senaste automatiska skanningen</small></article><article class="metric"><span>Kritiska händelser</span><strong class="critical">${num(critical)}</strong><small>i valt tidsfilter</small></article><article class="metric"><span>Varningar</span><strong class="warning">${num(warning)}</strong><small>i valt tidsfilter</small></article></section>
  <section class="panel monitor-findings-panel"><div class="panel-head"><div><span class="eyebrow">AKTIVA FLAGGOR</span><h2>Risker och fel som monitorn hittar</h2><p>Automatiska fynd från readiness, MFA, autentisering och säkerhetshändelser. Inga lösenord, IP-adresser, fingerprints eller hemligheter visas.</p></div><span class="panel-stat ${monitorTone}">${monitor.active===false?'Skanning fel':'Live'}</span></div>${securityFindings()}</section>
  ...
  <section class="dashboard-grid equal security-lower-grid">
    ...
    <article class="panel dashboard-panel"><div class="panel-head"><div><span class="eyebrow">ÅTGÄRDSLISTA</span><h2>Det som behöver uppmärksamhet</h2><p>Konkreta rekommendationer från readiness-kontroller som inte rapporterar OK.</p></div></div><div class="security-actions">${unresolvedSecurityActions()}</div><div class="security-privacy-note"><strong>Dataminimerad vy</strong><p>Säkerhetsportalen visar inte lösenord, MFA-hemligheter, IP-adresser, kundernas dokument eller ekonomiska detaljdata.</p></div></article>
  </section>`,'Säkerhetsportal','Aktiv riskjakt, systemhälsa, säkerhetshändelser och verifieringsbevis.');
}
