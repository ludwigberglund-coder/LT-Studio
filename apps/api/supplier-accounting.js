...
function postSupplierInvoice(db,{companyId,invoiceId,actorId}){
  ...
  return Db.transaction(db,()=>{
    ...
    if(existingOperation){
      ...
      return{invoice,entry,duplicate:true};
    }
    ...
    const update=db.prepare(`UPDATE supplier_invoices SET liability_accounting_entry_id=?,liability_posted_at=?,open_amount_ore=?,updated_at=? WHERE company_id=? AND id=? AND liability_accounting_entry_id IS NULL`).run(posted.entry.id,postedAt,invoice.totalOre,postedAt,companyId,invoice.id);
    ...
    Db.appendAudit(db,{companyId,userId:actorId,action:'SUPPLIER_INVOICE_POSTED',entityType:'supplier-invoice',entityId:invoice.id,details:{accountingEntryId:posted.entry.id,accountingNumber:posted.entry.number,totalOre:invoice.totalOre,openAmountOre:invoice.totalOre}});
    invoice=Payables.invoiceById(db,companyId,invoice.id);
    return{invoice,entry:posted.entry,duplicate:false};
  });
}

function paymentForConfirmation(db,companyId,paymentId){return db.prepare(`SELECT p.id,p.company_id AS companyId,p.supplier_invoice_id AS supplierInvoiceId,p.payment_date AS paymentDate,p.amount_ore AS amountOre,p.account,p.status,p.prepared_by AS preparedBy,p.released_by AS releasedBy,p.released_at AS releasedAt,p.confirmation_reference AS confirmationReference,p.accounting_entry_id AS accountingEntryId,p.paid_at AS paidAt,i.supplier_invoice_number AS supplierInvoiceNumber,i.status AS invoiceStatus,i.open_amount_ore AS openAmountOre,i.liability_accounting_entry_id AS liabilityAccountingEntryId,s.name AS supplierName FROM supplier_payments p JOIN supplier_invoices i ON i.id=p.supplier_invoice_id AND i.company_id=p.company_id JOIN suppliers s ON s.id=i.supplier_id AND s.company_id=i.company_id WHERE p.company_id=? AND p.id=?`).get(companyId,paymentId)||null}
...
function confirmSupplierPayment(db,{companyId,paymentId,confirmationReference,postingDate,actorId}){
  ...
  return Db.transaction(db,()=>{
    ...
    if(!payment.liabilityAccountingEntryId)throw flowError('Leverantörsskulden måste vara bokförd innan betalningen kan bokföras.','INVOICE_LIABILITY_NOT_POSTED',409);
    ...
    const invoiceUpdate=db.prepare(`UPDATE supplier_invoices SET status='paid',open_amount_ore=0,updated_at=? WHERE company_id=? AND id=? AND status='payment-prepared' AND open_amount_ore=? AND liability_accounting_entry_id IS NOT NULL`).run(paidAt,companyId,payment.supplierInvoiceId,payment.amountOre);
...
module.exports=Object.freeze({initializeSupplierAccounting,operationBySource,assertSupplierInvoiceCoding,postSupplierInvoice,paymentForConfirmation,confirmSupplierPayment});
