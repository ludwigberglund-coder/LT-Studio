...
function correctSupplierPayment(db,{companyId,paymentId,requestId,correctionDate,reason,actorId}){
  ...
  return Db.transaction(db,()=>{
    ...
    if(previous){
      ...
      return{payment:paymentForConfirmation(db,companyId,paymentId),invoice:Payables.invoiceById(db,companyId,paymentForConfirmation(db,companyId,paymentId)?.supplierInvoiceId),attempt:previous,reversal,duplicate:true};
    }
    ...
    payment=paymentForConfirmation(db,companyId,payment.id);
    return{payment,invoice:Payables.invoiceById(db,companyId,payment.supplierInvoiceId),attempt:correctionAttemptByRequest(db,companyId,key),originalEntry:original,reversal:reversed.entry,duplicate:false};
  });
}
