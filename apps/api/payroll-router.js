...
function createPayrollRouter(options){
  ...
  async function handle(req,res){
    ...
    try{
      ...
      if(url.pathname==='/api/v1/payroll/runs'&&req.method==='POST'){permission(s,'payroll.import');const body=await readJson(req,res);if(!body)return true;const run=Db.transaction(db,()=>{const value=Payroll.importRun(db,{...body,companyId:s.companyId,importedBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'PAYROLL_JOURNAL_IMPORTED',entityType:'payroll-run',entityId:value.id,details:{period:value.period,sourceName:value.sourceName,journalSha256:value.journalSha256}});return value});return send(res,201,{run}),true}
      const postMatch=url.pathname.match(/^\/api\/v1\/payroll\/runs\/([^/]+)\/post$/);
      if(postMatch&&req.method==='POST'){permission(s,'payroll.import');const result=Db.transaction(db,()=>{const value=Payroll.postRun(db,{companyId:s.companyId,runId:postMatch[1],postedBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'PAYROLL_JOURNAL_POSTED',entityType:'payroll-run',entityId:value.run.id,details:{period:value.run.period,entryId:value.entry.id,entryNumber:value.entry.number}});return value});return send(res,200,result),true}
