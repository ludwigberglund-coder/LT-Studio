...
function createInventoryRouter(options){
  ...
  async function handle(req,res){
    ...
    try{
      ...
      if(url.pathname==='/api/v1/inventory/items'&&req.method==='POST'){permission(s,'inventory.manage');const body=await readJson(req,res);if(!body)return true;const item=Db.transaction(db,()=>{const value=Inventory.createItem(db,{...body,companyId:s.companyId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'INVENTORY_ITEM_CREATED',entityType:'inventory-item',entityId:value.id,details:{sku:value.sku,name:value.name,unit:value.unit}});return value});return send(res,201,{item}),true}
      ...
      if(url.pathname==='/api/v1/inventory/movements'&&req.method==='POST'){permission(s,'inventory.manage');const body=await readJson(req,res);if(!body)return true;const created=Db.transaction(db,()=>{const value=Inventory.createMovement(db,{...body,companyId:s.companyId,actorId:s.userId});if(!value.duplicate)Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'INVENTORY_MOVEMENT_CREATED',entityType:'inventory-movement',entityId:value.movement.id,details:{itemId:value.movement.itemId,type:value.movement.type,quantityMilli:value.movement.quantityMilli,requestId:value.movement.requestId}});return value});return send(res,created.duplicate?200:201,created),true}
      ...
      if(url.pathname==='/api/v1/inventory/adjustments'&&req.method==='POST'){permission(s,'inventory.manage');const body=await readJson(req,res);if(!body)return true;const adjustment=Db.transaction(db,()=>{const value=Inventory.createAdjustment(db,{...body,companyId:s.companyId,countedBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'INVENTORY_ADJUSTMENT_REQUESTED',entityType:'inventory-adjustment',entityId:value.id,details:{itemId:value.itemId,differenceMilli:value.differenceMilli}});return value});return send(res,201,{adjustment}),true}
      const approve=url.pathname.match(/^\/api\/v1\/inventory\/adjustments\/([^/]+)\/approve$/);
      if(approve&&req.method==='POST'){permission(s,'inventory.adjust');const current=Inventory.adjustmentById(db,s.companyId,approve[1]);if(!current)throw routeError('Lagerjusteringen hittades inte.','ADJUSTMENT_NOT_FOUND',404);const workflow=Access.evaluateWorkflowAction(accessModel,s.actor,'inventory-adjustment',{countedBy:current.countedBy,approvedBy:s.userId});if(!workflow.allowed)throw routeError(workflow.reason,workflow.code,409);const result=Db.transaction(db,()=>{const value=Inventory.approveAdjustment(db,{companyId:s.companyId,adjustmentId:current.id,approvedBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'INVENTORY_ADJUSTMENT_APPROVED',entityType:'inventory-adjustment',entityId:current.id,details:{itemId:current.itemId,differenceMilli:current.differenceMilli,movementId:value.movement.id}});return value});return send(res,200,result),true}
      const reject=url.pathname.match(/^\/api\/v1\/inventory\/adjustments\/([^/]+)\/reject$/);
      if(reject&&req.method==='POST'){permission(s,'inventory.adjust');const adjustment=Db.transaction(db,()=>{const value=Inventory.rejectAdjustment(db,{companyId:s.companyId,adjustmentId:reject[1],rejectedBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'INVENTORY_ADJUSTMENT_REJECTED',entityType:'inventory-adjustment',entityId:value.id,details:{itemId:value.itemId,differenceMilli:value.differenceMilli}});return value});return send(res,200,{adjustment}),true}
