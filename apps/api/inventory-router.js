...
function createInventoryRouter(options){
  ...
  async function handle(req,res){
    ...
    try{
      ...
      if(url.pathname==='/api/v1/inventory/movements'&&req.method==='POST'){permission(s,'inventory.manage');const body=await readJson(req,res);if(!body)return true;const movement=Db.transaction(db,()=>{const value=Inventory.addMovement(db,{...body,companyId:s.companyId,actorId:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'INVENTORY_MOVEMENT_CREATED',entityType:'inventory-movement',entityId:value.id,details:{itemId:value.itemId,type:value.type,quantityMilli:value.quantityMilli}});return value});return send(res,201,{movement}),true}
