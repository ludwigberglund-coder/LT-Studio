...
function ensureState(db,companyId){let row=db.prepare(`SELECT company_id AS companyId,draft_site_json AS draftSiteJson,draft_company_json AS draftCompanyJson,draft_updated_by AS draftUpdatedBy,draft_updated_at AS draftUpdatedAt,published_site_json AS publishedSiteJson,published_company_json AS publishedCompanyJson,published_version AS publishedVersion,published_by AS publishedBy,published_at AS publishedAt FROM website_cms_state WHERE company_id=?`).get(companyId);if(!row){const initial=defaultContent(db,companyId),stamp=now();db.prepare(`INSERT INTO website_cms_state(company_id,draft_site_json,draft_company_json,draft_updated_at,published_site_json,published_company_json,published_version) VALUES(?,?,?,?,?,?,0)`).run(companyId,JSON.stringify(initial.site),JSON.stringify(initial.company),stamp,JSON.stringify(initial.site),JSON.stringify(initial.company));row=db.prepare(`SELECT company_id AS companyId,draft_site_json AS draftSiteJson,draft_company_json AS draftCompanyJson,draft_updated_by AS draftUpdatedBy,draft_updated_at AS draftUpdatedAt,published_site_json AS publishedSiteJson,published_company_json AS publishedCompanyJson,published_version AS publishedVersion,published_by AS publishedBy,published_at AS publishedAt FROM website_cms_state WHERE company_id=?`).get(companyId)}return row}
function state(db,companyId){const row=ensureState(db,companyId);return{companyId:row.companyId,draft:{site:parse(row.draftSiteJson,DEFAULT_SITE),company:parse(row.draftCompanyJson,DEFAULT_COMPANY),updatedBy:row.draftUpdatedBy,updatedAt:row.draftUpdatedAt},published:{site:parse(row.publishedSiteJson,DEFAULT_SITE),company:parse(row.publishedCompanyJson,DEFAULT_COMPANY),version:Number(row.publishedVersion||0),publishedBy:row.publishedBy,publishedAt:row.publishedAt}}}
