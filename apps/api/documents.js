...
function storeContent(db,{companyId,documentId,bytes}){const doc=documentById(db,companyId,documentId);if(!doc)throw documentError('Dokumentet hittades inte.','DOCUMENT_NOT_FOUND',404);if(doc.status==='ready')throw documentError('Ett färdigställt originaldokument kan inte ersättas.','DOCUMENT_IMMUTABLE',409);if(!Buffer.isBuffer(bytes)||!bytes.length)throw documentError('Dokumentinnehåll saknas.','MISSING_DOCUMENT_CONTENT');if(bytes.length>MAX_BYTES)throw documentError('Dokumentet får vara högst 15 MB.','DOCUMENT_TOO_LARGE',413);if(!magicMatches(doc.mimeType,bytes))throw documentError('Filens innehåll stämmer inte med angiven filtyp.','DOCUMENT_MAGIC_MISMATCH',415);const sha256=crypto.createHash('sha256').update(bytes).digest('hex');const duplicate=db.prepare(`SELECT id,title FROM documents WHERE company_id=? AND sha256=? AND id<>?`).get(companyId,sha256,documentId);if(duplicate)throw documentError(`Samma originalfil finns redan i dokumentarkivet (${duplicate.title}).`,'DUPLICATE_DOCUMENT',409);const completedAt=nowIso();db.prepare(`UPDATE documents SET sha256=?,size_bytes=?,content_blob=?,status='ready',completed_at=? WHERE company_id=? AND id=? AND status='pending'`).run(sha256,bytes.length,bytes,completedAt,companyId,documentId);return documentById(db,companyId,documentId)}
...
function content(db,companyId,documentId){const row=db.prepare(`SELECT file_name AS fileName,mime_type AS mimeType,sha256,size_bytes AS sizeBytes,content_blob AS bytes,status FROM documents WHERE company_id=? AND id=?`).get(companyId,documentId);if(!row||row.status!=='ready'||!row.bytes)throw documentError('Dokumentinnehållet hittades inte.','DOCUMENT_CONTENT_NOT_FOUND',404);return row}
...
module.exports=Object.freeze({ALLOWED_MIME,MAX_BYTES,initializeDocuments,createPending,storeContent,linkDocument,linksForDocument,documentById,content,listDocuments,magicMatches});
