...
function content(db,companyId,documentId){const row=db.prepare(`SELECT file_name AS fileName,mime_type AS mimeType,sha256,size_bytes AS sizeBytes,content_blob AS bytes,status FROM documents WHERE company_id=? AND id=?`).get(companyId,documentId);return verifyContent(row)}
