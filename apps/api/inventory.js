...
`);
  const movementColumns=db.prepare('PRAGMA table_info(inventory_movements)').all();
  if(!movementColumns.some(column=>column.name==='request_id'))db.exec('ALTER TABLE inventory_movements ADD COLUMN request_id TEXT');
  db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_inventory_movement_company_request ON inventory_movements(company_id,request_id) WHERE request_id IS NOT NULL');
}
...
function validRequestId(value){return /^[A-Za-z0-9:_-]{8,160}$/.test(text(value))}
function addMovement(db,{companyId,itemId,movementDate,type,quantityMilli,unitCostOre=null,referenceType='',referenceId='',note='',actorId,requestId=''}){const item=itemById(db,companyId,itemId);if(!item)throw inventoryError('Artikeln hittades inte.','ITEM_NOT_FOUND',404);if(!validDate(movementDate))throw inventoryError('Lagerdatumet är ogiltigt.','INVALID_MOVEMENT_DATE');if(!['receipt','sale','waste','adjustment'].includes(type))throw inventoryError('Okänd lagerrörelse.','INVALID_MOVEMENT_TYPE');if(!Number.isSafeInteger(quantityMilli)||quantityMilli===0)throw inventoryError('Kvantiteten måste anges i tusendelar och får inte vara noll.','INVALID_QUANTITY');if(type==='receipt'&&quantityMilli<0)throw inventoryError('En inleverans måste öka lagret.','INVALID_RECEIPT_QUANTITY');if(['sale','waste'].includes(type)&&quantityMilli>0)throw inventoryError('Försäljning och svinn måste minska lagret.','INVALID_OUTFLOW_QUANTITY');if(unitCostOre!==null&&(!Number.isSafeInteger(unitCostOre)||unitCostOre<0))throw inventoryError('Inköpspriset måste vara heltal i ören.','INVALID_UNIT_COST');const key=text(requestId);if(key&&!validRequestId(key))throw inventoryError('Idempotensnyckeln för lagerrörelsen är ogiltig.','INVALID_INVENTORY_REQUEST_ID');const current=balanceMilli(db,companyId,itemId);if(current+quantityMilli<0)throw inventoryError('Lagerrörelsen skulle ge negativt lagersaldo.','NEGATIVE_STOCK',409);const movementId=id('imov'),createdAt=nowIso();db.prepare(`INSERT INTO inventory_movements(id,company_id,item_id,movement_date,type,quantity_milli,unit_cost_ore,reference_type,reference_id,note,actor_id,created_at,request_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(movementId,companyId,itemId,movementDate,type,quantityMilli,unitCostOre,text(referenceType)||null,text(referenceId)||null,text(note).slice(0,500)||null,actorId,createdAt,key||null);return movementById(db,companyId,movementId)}
function movementById(db,companyId,movementId){return db.prepare(`SELECT id,company_id AS companyId,item_id AS itemId,movement_date AS movementDate,type,quantity_milli AS quantityMilli,unit_cost_ore AS unitCostOre,reference_type AS referenceType,reference_id AS referenceId,note,actor_id AS actorId,created_at AS createdAt,request_id AS requestId FROM inventory_movements WHERE company_id=? AND id=?`).get(companyId,movementId)||null}
function movementByRequestId(db,companyId,requestId){const row=db.prepare('SELECT id FROM inventory_movements WHERE company_id=? AND request_id=?').get(companyId,text(requestId));return row?movementById(db,companyId,row.id):null}
function createMovement(db,input){const companyId=text(input?.companyId),requestId=text(input?.requestId);if(!validRequestId(requestId))throw inventoryError('En giltig idempotensnyckel krävs för manuella lagerrörelser.','INVENTORY_REQUEST_ID_REQUIRED',422);const existing=movementByRequestId(db,companyId,requestId);if(existing){const unitCost=input.unitCostOre==null?null:Number(input.unitCostOre),referenceType=text(input.referenceType)||null,referenceId=text(input.referenceId)||null,note=text(input.note).slice(0,500)||null;const same=existing.itemId===text(input.itemId)&&existing.movementDate===text(input.movementDate)&&existing.type===text(input.type)&&existing.quantityMilli===Number(input.quantityMilli)&&existing.unitCostOre===unitCost&&(existing.referenceType||null)===referenceType&&(existing.referenceId||null)===referenceId&&(existing.note||null)===note;if(!same)throw inventoryError('Idempotensnyckeln används redan för en annan lagerrörelse.','INVENTORY_IDEMPOTENCY_CONFLICT',409);return{movement:existing,duplicate:true}}return{movement:addMovement(db,{...input,companyId,requestId}),duplicate:false}}
...
module.exports=Object.freeze({initializeInventory,createItem,itemById,listItems,balanceMilli,addMovement,createMovement,movementById,movementByRequestId,listMovements,createAdjustment,adjustmentById,listAdjustments,approveAdjustment,rejectAdjustment});
