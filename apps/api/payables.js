...
function createSupplierIdempotent(db,input){
  try{return{supplier:createSupplier(db,input),duplicate:false}}
  catch(error){
    if(error?.code!=='DUPLICATE_SUPPLIER')throw error;
    const existing=supplierByNumber(db,text(input.companyId),text(input.supplierNumber));
    if(!existing)throw error;
    const same=
      existing.name===text(input.name)&&
      (existing.orgNumber||null)===(text(input.orgNumber)||null)&&
      (existing.email||null)===(text(input.email)||null)&&
      (existing.bankgiro||null)===(text(input.bankgiro)||null)&&
      (existing.plusgiro||null)===(text(input.plusgiro)||null)&&
      (existing.defaultCostAccount||null)===(text(input.defaultCostAccount)||null);
    if(!same)throw err('Leverantörsnumret finns redan med andra leverantörsuppgifter. Ladda om och kontrollera leverantören.','SUPPLIER_CREATE_IDEMPOTENCY_CONFLICT',409,{existingSupplierId:existing.id});
    return{supplier:existing,duplicate:true};
  }
}
...
function createSupplierInvoiceIdempotent(db,input){
  try{return{invoice:createSupplierInvoice(db,input),duplicate:false}}
  catch(error){
    if(error?.code!=='DUPLICATE_SUPPLIER_INVOICE'||!error?.details?.existingInvoiceId)throw error;
    const existing=invoiceById(db,text(input.companyId),error.details.existingInvoiceId);
    if(!existing)throw error;
    const same=
      existing.supplierId===text(input.supplierId)&&
      normalizeInvoiceNumber(existing.supplierInvoiceNumber)===normalizeInvoiceNumber(input.supplierInvoiceNumber)&&
      existing.invoiceDate===text(input.invoiceDate)&&
      existing.dueDate===text(input.dueDate)&&
      existing.totalOre===Number(input.totalOre)&&
      existing.vatOre===Number(input.vatOre||0)&&
      existing.currency===text(input.currency||'SEK').toUpperCase()&&
      (existing.vatTreatment||null)===(text(input.vatTreatment)||null);
    if(!same)throw err('Samma leverantör och fakturanummer finns redan med andra fakturauppgifter. Ladda om och kontrollera originalet.','SUPPLIER_INVOICE_IDEMPOTENCY_CONFLICT',409,{existingInvoiceId:existing.id});
...
function storeDocumentIdempotent(db,input){
  const companyId=text(input.companyId),invoiceId=text(input.invoiceId),mime=input.mime||'application/pdf',name=text(input.name)||'leverantorsfaktura.pdf',bytes=input.bytes;
  const invoice=invoiceById(db,companyId,invoiceId);
  if(!invoice)throw err('Leverantörsfakturan hittades inte.','INVOICE_NOT_FOUND',404);
  if(!Buffer.isBuffer(bytes)||!bytes.length)throw err('PDF-dokument saknas.','MISSING_DOCUMENT');
  if(mime!=='application/pdf')throw err('Endast PDF stöds för leverantörsfakturor i denna version.','UNSUPPORTED_DOCUMENT_TYPE');
  ...
  if(invoice.documentSha256===sha&&invoice.documentName===name&&invoice.documentMime===mime){
    ...
    if(existing.sha256!==sha||existing.bytes.length!==bytes.length)throw err('PDF-underlaget stämmer inte med sitt sparade digitala fingeravtryck.','DOCUMENT_INTEGRITY_ERROR',409);
    return{document:{sha256:sha,size:bytes.length},duplicate:true};
  }
  return{document:storeDocument(db,{...input,companyId,invoiceId,name,mime,bytes}),duplicate:false};
}
...
module.exports=Object.freeze({initializePayables,listSuppliers,createSupplier,createSupplierIdempotent,supplierById,supplierByNumber,createSupplierInvoice,createSupplierInvoiceIdempotent,invoiceById,listInvoices,supplierHistory,storeDocument,storeDocumentIdempotent,document,privateObjectMetadata,saveCoding,saveCodingIdempotent,approve,approveIdempotent,preparePayment,paymentByInvoice,paymentById,listPayments,validDate,normalizeInvoiceNumber,duplicateInvoice,duplicateDocument});
