...
function createSupplier(db,input){
  const companyId=text(input.companyId),supplierNumber=text(input.supplierNumber),name=text(input.name),defaultCostAccount=text(input.defaultCostAccount);
  if(!companyId||supplierNumber.length<1||supplierNumber.length>40||name.length<2||name.length>160)throw err('Företag, leverantörsnummer och namn krävs.','INVALID_SUPPLIER');
  if(defaultCostAccount&&!validAccount(defaultCostAccount))throw err('Standardkontot måste bestå av fyra siffror.','INVALID_SUPPLIER_ACCOUNT');
  if(supplierByNumber(db,companyId,supplierNumber))throw err('Leverantörsnumret används redan.','DUPLICATE_SUPPLIER',409);
  const now=nowIso(),supplierId=input.id||id('supplier');
  db.prepare(`INSERT INTO suppliers(id,company_id,supplier_number,name,org_number,email,bankgiro,plusgiro,default_cost_account,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(supplierId,companyId,supplierNumber,name,text(input.orgNumber)||null,text(input.email)||null,text(input.bankgiro)||null,text(input.plusgiro)||null,defaultCostAccount||null,now,now);
  return supplierById(db,companyId,supplierId);
}
...
function listInvoices(db,companyId){return db.prepare(`SELECT i.id,i.company_id AS companyId,i.supplier_id AS supplierId,i.supplier_invoice_number AS supplierInvoiceNumber,i.invoice_date AS invoiceDate,i.due_date AS dueDate,i.total_ore AS totalOre,i.vat_ore AS vatOre,i.currency,i.status,i.registered_by AS registeredBy,i.approved_by AS approvedBy,i.approved_at AS approvedAt,CASE WHEN i.document_sha256 IS NULL THEN 0 ELSE 1 END AS hasDocument,s.supplier_number AS supplierNumber,s.name AS supplierName,s.bankgiro,s.plusgiro FROM supplier_invoices i JOIN suppliers s ON s.id=i.supplier_id AND s.company_id=i.company_id WHERE i.company_id=? ORDER BY i.due_date,i.created_at`).all(companyId).map(row=>({...row,hasDocument:Boolean(row.hasDocument)}))}
function duplicateInvoice(db,companyId,supplierId,number){return db.prepare(`SELECT id FROM supplier_invoices WHERE company_id=? AND supplier_id=? AND supplier_invoice_number=?`).get(companyId,supplierId,text(number))||null}
function createSupplierInvoice(db,input){
  const companyId=text(input.companyId),supplierId=text(input.supplierId),number=text(input.supplierInvoiceNumber),invoiceDate=text(input.invoiceDate),dueDate=text(input.dueDate),currency=text(input.currency||'SEK').toUpperCase();
  if(!companyId||!supplierId||number.length<1||number.length>100)throw err('Företag, leverantör och fakturanummer krävs.','INVALID_SUPPLIER_INVOICE');
  if(!supplierById(db,companyId,supplierId))throw err('Leverantören hittades inte i företaget.','SUPPLIER_NOT_FOUND',404);
  if(!validDate(invoiceDate)||!validDate(dueDate)||dueDate<invoiceDate)throw err('Fakturadatum eller förfallodatum är ogiltigt.','INVALID_SUPPLIER_INVOICE_DATE');
  if(!Number.isSafeInteger(input.totalOre)||input.totalOre<=0||!Number.isSafeInteger(input.vatOre||0)||input.vatOre<0||input.vatOre>input.totalOre)throw err('Fakturabelopp och moms måste anges som giltiga heltal i ören.','INVALID_SUPPLIER_INVOICE_AMOUNT');
  if(currency!=='SEK')throw err('Den första versionen av leverantörsfakturor stöder endast SEK.','UNSUPPORTED_CURRENCY');
  if(duplicateInvoice(db,companyId,supplierId,number))throw err('Leverantörsfakturan är redan registrerad.','DUPLICATE_SUPPLIER_INVOICE',409);
  const now=nowIso(),invoiceId=input.id||id('sinv');
  db.prepare(`INSERT INTO supplier_invoices(id,company_id,supplier_id,supplier_invoice_number,invoice_date,due_date,total_ore,vat_ore,currency,status,registered_by,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,'registered',?,?,?)`).run(invoiceId,companyId,supplierId,number,invoiceDate,dueDate,input.totalOre,input.vatOre||0,currency,input.registeredBy,now,now);
...
function storeDocument(db,{companyId,invoiceId,name,mime='application/pdf',bytes}){
  const invoice=invoiceById(db,companyId,invoiceId);if(!invoice)throw err('Leverantörsfakturan hittades inte.','INVOICE_NOT_FOUND',404);
  ...
  db.prepare(`UPDATE supplier_invoices SET document_name=?,document_mime=?,document_sha256=?,document_blob=?,updated_at=? WHERE company_id=? AND id=?`).run(text(name)||'leverantorsfaktura.pdf',mime,sha,bytes,nowIso(),companyId,invoiceId);
...
module.exports=Object.freeze({initializePayables,listSuppliers,createSupplier,supplierById,supplierByNumber,createSupplierInvoice,invoiceById,listInvoices,supplierHistory,storeDocument,document,saveCoding,approve,preparePayment,paymentById,listPayments,validDate});
