...
function createSqlitePaymentReminderPdfArchiveStore(db){
  ...
  function exists({companyId,reminderId}){
    ensureSchema();
    return Boolean(db.prepare('SELECT 1 AS present FROM payment_reminder_pdf_archives WHERE company_id=? AND reminder_id=?').get(companyId,reminderId));
  }
