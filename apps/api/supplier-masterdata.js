...
function requestChange(db,{companyId,supplierId,kind,changes,requestedBy}){
  ...
  const clean=validateChanges(kind,changes);
  ...
  db.prepare(`INSERT INTO supplier_change_requests(id,company_id,supplier_id,kind,changes_json,status,requested_by,requested_at) VALUES(?,?,?,?,?,'pending',?,?)`).run(requestId,companyId,supplierId,kind,JSON.stringify(clean),requestedBy,requestedAt);
  if(kind==='profile') return approveProfileChange(db,{companyId,requestId,actorId:requestedBy});
  return changeRequestById(db,companyId,requestId);
}
function changeRequestById(db,companyId,requestId){const row=db.prepare(`SELECT id,company_id AS companyId,supplier_id AS supplierId,kind,changes_json AS changesJson,status,requested_by AS requestedBy,requested_at AS requestedAt,approved_by AS approvedBy,approved_at AS approvedAt,rejected_by AS rejectedBy,rejected_at AS rejectedAt,decision_reason AS decisionReason FROM supplier_change_requests WHERE company_id=? AND id=?`).get(companyId,requestId);return row?{...row,changes:JSON.parse(row.changesJson)}:null}
...
module.exports=Object.freeze({initializeSupplierMasterdata,requestChange,changeRequestById,listPending,approvePaymentChange,rejectPaymentChange,history,snapshotPaymentRecipient,validateChanges});
