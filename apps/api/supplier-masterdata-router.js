...
function createSupplierMasterdataRouter(options){
  ...
  async function handle(req,res){
    ...
    try{
      ...
      if(profileMatch&&req.method==='PUT'){permission(s,'supplier.manage');const payload=await readJson(req,res);if(!payload)return true;const request=Db.transaction(db,()=>{const value=Master.requestChange(db,{companyId:s.companyId,supplierId:profileMatch[1],kind:'profile',changes:payload,requestedBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'SUPPLIER_PROFILE_UPDATED',entityType:'supplier',entityId:profileMatch[1],details:{requestId:value.id}});return value});return send(res,200,{request}),true}
      const bankMatch=url.pathname.match(/^\/api\/v1\/suppliers\/([^/]+)\/payment-details$/);
      if(bankMatch&&req.method==='POST'){permission(s,'supplier.manage');const payload=await readJson(req,res);if(!payload)return true;const request=Db.transaction(db,()=>{const value=Master.requestChange(db,{companyId:s.companyId,supplierId:bankMatch[1],kind:'payment-details',changes:payload,requestedBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'SUPPLIER_PAYMENT_DETAILS_REQUESTED',entityType:'supplier',entityId:bankMatch[1],details:{requestId:value.id}});return value});return send(res,202,{request,message:'Ändringen väntar på separat godkännande.'}),true}
