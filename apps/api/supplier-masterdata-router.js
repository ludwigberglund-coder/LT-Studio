...
function createSupplierMasterdataRouter(options){
  ...
  async function handle(req,res){
    ...
    try{
      ...
      if(bankMatch&&req.method==='POST'){permission(s,'supplier.manage');const payload=await readJson(req,res);if(!payload)return true;const result=Db.transaction(db,()=>{const value=Master.requestChangeIdempotent(db,{companyId:s.companyId,supplierId:bankMatch[1],kind:'payment-details',changes:payload,requestedBy:s.userId,requestKey:payload.requestId});if(!value.duplicate)Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'SUPPLIER_PAYMENT_DETAILS_UPDATED',entityType:'supplier',entityId:bankMatch[1],details:{requestId:value.request.id,requestKey:payload.requestId}});return value});return send(res,200,{...result,message:'Betalningsuppgifterna har sparats.'}),true}
