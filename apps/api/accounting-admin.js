...
function adminError(message,code='ACCOUNTING_ADMIN_ERROR',statusCode=422){const e=new Error(message);e.code=code;e.statusCode=statusCode;return e}
...
function initializeAccountingAdmin(db){Accounting.initializeAccountingStore(db);db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS period_unlock_requests(
    ...
    UNIQUE(company_id,period,status)
  ) STRICT;
...
function periodStatus(db,companyId,period){if(!validPeriod(period))throw adminError('Perioden måste anges som ÅÅÅÅ-MM.','INVALID_PERIOD');const row=db.prepare(`SELECT period,status,locked_by AS lockedBy,locked_at AS lockedAt FROM accounting_periods WHERE company_id=? AND period=?`).get(companyId,period);return row||{period,status:'open',lockedBy:null,lockedAt:null}}
function listPeriods(db,companyId,{year=''}={}){const prefix=/^\d{4}$/.test(text(year))?`${year}-%`:'%';return db.prepare(`SELECT period,status,locked_by AS lockedBy,locked_at AS lockedAt FROM accounting_periods WHERE company_id=? AND period LIKE ? ORDER BY period DESC`).all(companyId,prefix)}
function lockPeriod(db,{companyId,period,lockedBy}){if(!validPeriod(period))throw adminError('Perioden måste anges som ÅÅÅÅ-MM.','INVALID_PERIOD');const current=periodStatus(db,companyId,period);if(current.status==='locked')throw adminError('Perioden är redan låst.','PERIOD_ALREADY_LOCKED',409);const now=nowIso();db.prepare(`INSERT INTO accounting_periods(company_id,period,status,locked_by,locked_at) VALUES(?,?,'locked',?,?) ON CONFLICT(company_id,period) DO UPDATE SET status='locked',locked_by=excluded.locked_by,locked_at=excluded.locked_at`).run(companyId,period,lockedBy,now);return periodStatus(db,companyId,period)}
function requestUnlock(db,{companyId,period,reason,requestedBy}){if(periodStatus(db,companyId,period).status!=='locked')throw adminError('Endast en låst period kan begäras upplåst.','PERIOD_NOT_LOCKED',409);const clean=text(reason);if(clean.length<5||clean.length>500)throw adminError('En tydlig orsak på 5–500 tecken krävs.','UNLOCK_REASON_REQUIRED');const pending=db.prepare(`SELECT id FROM period_unlock_requests WHERE company_id=? AND period=? AND status='pending'`).get(companyId,period);if(pending)throw adminError('Det finns redan en väntande upplåsningsbegäran.','UNLOCK_ALREADY_PENDING',409);const requestId=id('unlock'),requestedAt=nowIso();db.prepare(`INSERT INTO period_unlock_requests(id,company_id,period,reason,status,requested_by,requested_at) VALUES(?,?,?,?,'pending',?,?)`).run(requestId,companyId,period,clean,requestedBy,requestedAt);return unlockRequestById(db,companyId,requestId)}
...
function decideUnlock(db,{companyId,requestId,decidedBy,decision,decisionReason=''}){const req=unlockRequestById(db,companyId,requestId);if(!req)throw adminError('Upplåsningsbegäran hittades inte.','UNLOCK_REQUEST_NOT_FOUND',404);if(req.status!=='pending')throw adminError('Begäran är redan behandlad.','UNLOCK_ALREADY_DECIDED',409);if(req.requestedBy===decidedBy)throw adminError('Den som begärde upplåsningen får inte godkänna den själv.','SEPARATION_OF_DUTIES_FAILED',409);if(!['approved','rejected'].includes(decision))throw adminError('Beslutet är ogiltigt.','INVALID_UNLOCK_DECISION');const decidedAt=nowIso();if(decision==='approved'){db.prepare(`UPDATE accounting_periods SET status='open',locked_by=NULL,locked_at=NULL WHERE company_id=? AND period=? AND status='locked'`).run(companyId,req.period)}db.prepare(`UPDATE period_unlock_requests SET status=?,decided_by=?,decided_at=?,decision_reason=? WHERE company_id=? AND id=? AND status='pending'`).run(decision,decidedBy,decidedAt,text(decisionReason).slice(0,500)||null,companyId,requestId);return{request:unlockRequestById(db,companyId,requestId),period:periodStatus(db,companyId,req.period)}}
...
module.exports=Object.freeze({initializeAccountingAdmin,entryById,periodStatus,listPeriods,lockPeriod,requestUnlock,unlockRequestById,listUnlockRequests,decideUnlock,correctionByOriginal,correctEntry,listCorrections,validPeriod});
