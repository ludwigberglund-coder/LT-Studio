...
function paymentOverview(db,companyId,{mode='month',date,status='',direction=''}={}){
  const bounds=periodBounds({mode,date});
  const normalizedStatus=text(status).toLowerCase(),normalizedDirection=text(direction).toLowerCase();
  ...
  const incoming=db.prepare(`SELECT b.id,'in' AS direction,b.booking_date AS paymentDate,b.amount_ore AS amountOre,b.status,
    b.reference,b.payer_name AS counterparty,b.payer_account AS counterpartyAccount,NULL AS invoiceNumber
  ...
  const outgoing=db.prepare(`SELECT p.id,'out' AS direction,p.payment_date AS paymentDate,p.amount_ore AS amountOre,p.status,
    ...
    COALESCE(p.recipient_bankgiro,p.recipient_plusgiro,'') AS counterpartyAccount,i.supplier_invoice_number AS invoiceNumber
  ...
  const rows=[...incoming,...outgoing].filter(row=>(!normalizedStatus||String(row.status).toLowerCase()===normalizedStatus)&&(!normalizedDirection||row.direction===normalizedDirection))
    .sort((a,b)=>a.paymentDate.localeCompare(b.paymentDate)||a.direction.localeCompare(b.direction)||a.id.localeCompare(b.id));
  ...
  return{period:bounds,filters:{status:normalizedStatus||null,direction:normalizedDirection||null},summary:{incomingOre,outgoingOre,netOre:incomingOre-outgoingOre,count:rows.length},rows};
}
