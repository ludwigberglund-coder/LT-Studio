...
function assertSyntheticStagingDatabase(db,env=process.env){
  ...
  const rows=db.prepare(
    'SELECT id,legal_name AS legalName,display_name AS displayName,org_number AS orgNumber FROM companies ORDER BY org_number'
  ).all();

  if(rows.length!==EXPECTED_STAGING_COMPANIES.length){
    throw stagingDataError(
      `Stagingdatabasen måste innehålla exakt ${EXPECTED_STAGING_COMPANIES.length} syntetiska företag. Hittade ${rows.length}.`
    );
  }
  ...
  for(const row of rows){
    ...
    if(!expected||
      row.legalName!==expected.legalName||
      row.displayName!==expected.displayName){
      throw stagingDataError(
        'Stagingdatabasen innehåller ett företag som inte matchar den godkända syntetiska fixture-identiteten.'
      );
    }

    const audits=db.prepare(
      "SELECT details_json AS detailsJson FROM audit_events WHERE company_id=? AND action='SYNTHETIC_STAGING_BOOTSTRAP'"
    ).all(row.id);
    const hasSyntheticBootstrap=audits.some(audit=>{
      try{return JSON.parse(audit.detailsJson||'{}').dataClassification==='synthetic'}
      catch{return false}
    });
    if(!hasSyntheticBootstrap){
      throw stagingDataError(
        `Stagingföretaget ${row.orgNumber} saknar verifierbar SYNTHETIC_STAGING_BOOTSTRAP-audit.`
      );
    }
  }
...
module.exports=Object.freeze({
  EXPECTED_STAGING_COMPANIES,
  assertSyntheticStagingDatabase
});
