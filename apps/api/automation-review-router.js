    ...
    try {
      ...
      if (approveMatch && req.method === 'POST') {
        ...
        const result = Db.transaction(db,()=>{
          const value = Queues.approveAutomationProposalIdempotent(db,{companyId:session.companyId,proposalId:existing.id,userId:session.userId});
          const proposal=value.proposal;
          if(!value.duplicate&&proposal.type==='bank-payment-match'){
          ...
          if(!value.duplicate)Db.appendAudit(db,{companyId:session.companyId,userId:session.userId,action:'AUTOMATION_PROPOSAL_APPROVED',entityType:'automation-proposal',entityId:proposal.id,details:{proposalType:proposal.type,sourceId:proposal.sourceId,suggestion:proposal.suggestion,executionStatus:'not-executed'}});
          return{proposal:ReviewService.byIdForReview(db,session.companyId,proposal.id),duplicate:value.duplicate};
        });
        return send(res,200,{...result,executionStatus:'not-executed',message:result.duplicate?'Förslaget var redan godkänt av samma användare. Ingen ny åtgärd eller auditpost skapades.':'Förslaget är godkänt för nästa kontrollerade steg men har inte bokförts eller betalats automatiskt.'}), true;
      }
      ...
      if (rejectMatch && req.method === 'POST') {
        ...
        const payload = await readJson(req,res); if (!payload) return true;
        const result = Db.transaction(db,()=>{
          const value = Queues.rejectAutomationProposalIdempotent(db,{companyId:session.companyId,proposalId:existing.id,userId:session.userId,reason:payload.reason});
          const proposal=value.proposal;
          if(!value.duplicate)Db.appendAudit(db,{companyId:session.companyId,userId:session.userId,action:'AUTOMATION_PROPOSAL_REJECTED',entityType:'automation-proposal',entityId:proposal.id,details:{proposalType:proposal.type,sourceId:proposal.sourceId,reason:proposal.rejectionReason}});
          return{proposal:ReviewService.byIdForReview(db,session.companyId,proposal.id),duplicate:value.duplicate};
        });
        return send(res,200,{...result,executionStatus:'not-executed',message:result.duplicate?'Förslaget var redan avslaget av samma användare med samma orsak. Ingen ny auditpost skapades.':'Förslaget har avslagits.'}), true;
      }
