...
function createServer(options = {}) {
  ...
  const readinessPayload=()=>{
    const report=readinessReport({db,databasePath,backupPath:process.env.ROLLANDS_BACKUP_PATH||'',offsiteBackupEvidencePath:process.env.ROLLANDS_OFFSITE_BACKUP_EVIDENCE_PATH||'',restoreEvidencePath:process.env.ROLLANDS_RESTORE_DRILL_EVIDENCE_PATH||'',offsiteRestoreEvidencePath:process.env.ROLLANDS_OFFSITE_RESTORE_EVIDENCE_PATH||'',monitoringEvidencePath:process.env.ROLLANDS_MONITORING_EVIDENCE_PATH||'',requireBackup:protectedMode,requireOffsiteBackupEvidence:protectedMode,requireRestoreEvidence:protectedMode,requireOffsiteRestoreEvidence:protectedMode,requireMonitoringEvidence:protectedMode});
    return {ok:report.ok,service:'rollands-api-v1',checks:report.checks,freeMiB:report.freeBytes===null?null:Math.floor(report.freeBytes/1048576),backupAgeMinutes:report.backupAgeMs===null?null:Math.floor(report.backupAgeMs/60000),offsiteBackupAgeMinutes:report.offsiteBackupAgeMs===null?null:Math.floor(report.offsiteBackupAgeMs/60000),restoreDrillAgeMinutes:report.restoreDrillAgeMs===null?null:Math.floor(report.restoreDrillAgeMs/60000),offsiteRestoreAgeMinutes:report.offsiteRestoreAgeMs===null?null:Math.floor(report.offsiteRestoreAgeMs/60000),monitoringAgeMinutes:report.monitoringAgeMs===null?null:Math.floor(report.monitoringAgeMs/60000),alertTestAgeMinutes:report.alertAgeMs===null?null:Math.floor(report.alertAgeMs/60000)};
  };
