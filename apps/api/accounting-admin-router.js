...
function createAccountingAdminRouter(options){
 const db=options?.db;if(!db)throw new Error('Databas krävs.');Admin.initializeAccountingAdmin(db);const accessModel=Access.createModel(options.accessConfig||DEFAULT_ACCESS);const authEncryptionKey=String(options?.authEncryptionKey||'');
 ...
 function unlockEligibleUsers(companyId){
   return Db.membershipsForCompany(db,companyId).filter(member=>{
     if(member.disabled||member.platformAdmin)return false;
     const actor={id:member.userId,name:member.displayName,companyId,authenticated:true,membershipActive:true,disabled:false,role:member.role};
     return Access.authorize(accessModel,actor,'period.unlock').allowed;
   });
 }
 function verifySelfUnlock(sessionValue,body){
   const user=Db.userById(db,sessionValue.userId);
   if(!user||user.disabled)throw routeError('Användarkontot hittades inte.','USER_NOT_FOUND',404);
   if(!Auth.verifyPassword(body.password,user.passwordHash))throw routeError('Lösenordet är felaktigt.','REAUTH_PASSWORD_INVALID',401);
   if(!user.mfaSecretEncrypted)throw routeError('MFA krävs för självupplåsning.','MFA_ENROLLMENT_REQUIRED',403);
   if(authEncryptionKey.length<32)throw routeError('MFA kan inte verifieras eftersom serverns krypteringsnyckel saknas.','MFA_SERVER_NOT_CONFIGURED',503);
   let secret;try{secret=Auth.decryptSecret(user.mfaSecretEncrypted,authEncryptionKey)}catch{throw routeError('MFA-konfigurationen kan inte läsas.','MFA_SERVER_ERROR',503)}
   const counter=Auth.totpMatchCounter(secret,body.totp);
   if(counter===null)throw routeError('MFA-koden är felaktig eller har gått ut.','INVALID_MFA',401);
   return{userId:user.id,totpCounter:counter};
 }
 async function handle(req,res){
  ...
  try{const s=requireSession(req);if(req.method!=='GET')csrf(req,s);
   ...
   if(url.pathname==='/api/v1/accounting/unlock-requests'&&req.method==='GET'){permission(s,'accounting.view');const eligible=unlockEligibleUsers(s.companyId);return send(res,200,{requests:Admin.listUnlockRequests(db,s.companyId,{status:String(url.searchParams.get('status')||'all')}),unlockPolicy:{eligibleCustomerApprovers:eligible.length,selfUnlockAllowed:eligible.length===1&&eligible[0].userId===s.userId}}),true}
   const decide=url.pathname.match(/^\/api\/v1\/accounting\/unlock-requests\/([^/]+)\/(approve|reject)$/);if(decide&&req.method==='POST'){permission(s,'period.unlock');const current=Admin.unlockRequestById(db,s.companyId,decide[1]);if(!current)throw routeError('Upplåsningsbegäran hittades inte.','UNLOCK_REQUEST_NOT_FOUND',404);const body=await readJson(req,res);if(!body)return true;const eligible=unlockEligibleUsers(s.companyId);const selfDecision=current.requestedBy===s.userId;let selfVerified=false;if(selfDecision){if(!(eligible.length===1&&eligible[0].userId===s.userId))throw routeError('En annan behörig användare måste fatta beslutet när fler än en upplåsningsbehörig kundanvändare finns.','SEPARATION_OF_DUTIES_FAILED',409);if(decide[2]!=='approve')throw routeError('Självupplåsning gäller endast godkännande av den egna begäran.','SELF_UNLOCK_REJECT_NOT_ALLOWED',409);if(String(body.reason||'').trim().length<5)throw routeError('Ange en tydlig beslutsorsak för självupplåsningen.','SELF_UNLOCK_REASON_REQUIRED',422);selfVerified=verifySelfUnlock(s,body)}else{const workflow=Access.evaluateWorkflowAction(accessModel,s.actor,'period-unlock',{requestedBy:current.requestedBy,unlockedBy:s.userId});if(!workflow.allowed)throw routeError(workflow.reason,workflow.code,409)}const result=Db.transaction(db,()=>{if(selfVerified)Db.consumeMfaStep(db,selfVerified);const value=Admin.decideUnlock(db,{companyId:s.companyId,requestId:current.id,decidedBy:s.userId,decision:decide[2]==='approve'?'approved':'rejected',decisionReason:body.reason||'',allowSelfApproval:Boolean(selfVerified)});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:decide[2]==='approve'?'ACCOUNTING_PERIOD_UNLOCKED':'ACCOUNTING_PERIOD_UNLOCK_REJECTED',entityType:'accounting-period',entityId:current.period,details:{requestId:current.id,decision:value.request.status,selfUnlock:selfDecision,reauthenticated:Boolean(selfVerified),eligibleCustomerApprovers:eligible.length}});return value});return send(res,200,{...result,selfUnlock:selfDecision}),true}
