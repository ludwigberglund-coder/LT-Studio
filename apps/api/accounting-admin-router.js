...
function createAccountingAdminRouter(options){
 ...
 async function handle(req,res){
  ...
  try{const s=requireSession(req);if(req.method!=='GET')csrf(req,s);
   ...
   const entry=url.pathname.match(/^\/api\/v1\/accounting\/entries\/([^/]+)$/);if(entry&&req.method==='GET'){permission(s,'accounting.view');const value=Admin.entryById(db,s.companyId,entry[1]);if(!value)throw routeError('Verifikationen hittades inte.','ENTRY_NOT_FOUND',404);return send(res,200,{entry:value,correction:Admin.correctionByOriginal(db,s.companyId,value.id)}),true}
   const correction=url.pathname.match(/^\/api\/v1\/accounting\/entries\/([^/]+)\/correct$/);if(correction&&req.method==='POST'){permission(s,'accounting.correct');const body=await readJson(req,res);if(!body)return true;const result=Db.transaction(db,()=>{const value=Admin.correctEntry(db,{companyId:s.companyId,entryId:correction[1],postingDate:body.postingDate,reason:body.reason,replacementLines:body.replacementLines,createdBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'ACCOUNTING_ENTRY_CORRECTED',entityType:'accounting-entry',entityId:value.original.id,details:{correctionId:value.correction.id,reversalEntryId:value.reversal.id,replacementEntryId:value.replacement?.id||null,reason:value.correction.reason}});return value});return send(res,201,result),true}
   ...
   const lock=url.pathname.match(/^\/api\/v1\/accounting\/periods\/(\d{4}-\d{2})\/lock$/);if(lock&&req.method==='POST'){permission(s,'period.lock');const period=Db.transaction(db,()=>{const value=Admin.lockPeriod(db,{companyId:s.companyId,period:lock[1],lockedBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'ACCOUNTING_PERIOD_LOCKED',entityType:'accounting-period',entityId:value.period,details:{status:value.status}});return value});return send(res,200,{period}),true}
   const request=url.pathname.match(/^\/api\/v1\/accounting\/periods\/(\d{4}-\d{2})\/unlock-request$/);if(request&&req.method==='POST'){permission(s,'accounting.correct');const body=await readJson(req,res);if(!body)return true;const unlockRequest=Db.transaction(db,()=>{const value=Admin.requestUnlock(db,{companyId:s.companyId,period:request[1],reason:body.reason,requestedBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'ACCOUNTING_PERIOD_UNLOCK_REQUESTED',entityType:'accounting-period',entityId:value.period,details:{requestId:value.id,reason:value.reason}});return value});return send(res,201,{request:unlockRequest}),true}
