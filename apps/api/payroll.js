...
function normalizedImport(input){const companyId=text(input.companyId),period=text(input.period),payDate=text(input.payDate),sourceName=text(input.sourceName);if(!companyId||!validPeriod(period)||!validDate(payDate)||!sourceName||sourceName.length>120)throw payrollError('Företag, löneperiod, utbetalningsdatum och källa krävs.','INVALID_PAYROLL_RUN');validateSummary(input);const lines=normalizeLines(input.lines);return{companyId,period,payDate,sourceName,grossSalaryOre:Number(input.grossSalaryOre||0),withheldTaxOre:Number(input.withheldTaxOre||0),employerContributionsOre:Number(input.employerContributionsOre||0),netPayOre:Number(input.netPayOre||0),vacationLiabilityChangeOre:Number(input.vacationLiabilityChangeOre||0),lines,journalSha256:hashLines(lines),importedBy:text(input.importedBy)}}
function sameImport(run,value){return run.payDate===value.payDate&&run.grossSalaryOre===value.grossSalaryOre&&run.withheldTaxOre===value.withheldTaxOre&&run.employerContributionsOre===value.employerContributionsOre&&run.netPayOre===value.netPayOre&&run.vacationLiabilityChangeOre===value.vacationLiabilityChangeOre}
function importRunIdempotent(db,input){const value=normalizedImport(input);const existing=db.prepare(`SELECT id FROM payroll_runs WHERE company_id=? AND period=? AND source_name=? AND journal_sha256=?`).get(value.companyId,value.period,value.sourceName,value.journalSha256);if(existing){const run=runById(db,value.companyId,existing.id);if(!sameImport(run,value))throw payrollError('Samma lönejournalidentitet finns redan med andra sammanfattningsbelopp eller annat utbetalningsdatum. Ladda om och kontrollera underlaget.','PAYROLL_IMPORT_IDEMPOTENCY_CONFLICT',409);return{run,duplicate:true}}const runId=input.id||id('payroll'),importedAt=nowIso();db.prepare(`INSERT INTO payroll_runs(id,company_id,period,pay_date,source_name,gross_salary_ore,withheld_tax_ore,employer_contributions_ore,net_pay_ore,vacation_liability_change_ore,lines_json,journal_sha256,status,imported_by,imported_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,'validated',?,?)`).run(runId,value.companyId,value.period,value.payDate,value.sourceName,value.grossSalaryOre,value.withheldTaxOre,value.employerContributionsOre,value.netPayOre,value.vacationLiabilityChangeOre,JSON.stringify(value.lines),value.journalSha256,value.importedBy,importedAt);return{run:runById(db,value.companyId,runId),duplicate:false}}
function importRun(db,input){const result=importRunIdempotent(db,input);if(result.duplicate)throw payrollError('Samma lönejournal är redan importerad för perioden.','DUPLICATE_PAYROLL_RUN',409);return result.run}
