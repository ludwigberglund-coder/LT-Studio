...
function createPayablesRouter(options){
  const db=options?.db;if(!db)throw new Error('Databas krävs.');Payables.initializePayables(db);Queues.initializeQueues(db);Accounting.initializeAccountingStore(db);
  ...
  async function handle(req,res){
    ...
    try{
      ...
      if(invoiceMatch&&req.method==='GET'){permission(s,'supplier-invoice.view');const invoice=Payables.invoiceById(db,s.companyId,invoiceMatch[1]);if(!invoice)throw routeError('Fakturan hittades inte.','INVOICE_NOT_FOUND',404);const accountingEntry=Accounting.entryBySource(db,s.companyId,'supplier-invoice',invoice.id);return send(res,200,{invoice:{...invoice,accountingPosted:Boolean(accountingEntry),accountingEntryNumber:accountingEntry?.number||null}}),true}
      ...
      if(payMatch&&req.method==='POST'){
        permission(s,'payment.prepare');permission(s,'accounting.post');const payload=await readJson(req,res);if(!payload)return true;const invoice=Payables.invoiceById(db,s.companyId,payMatch[1]);if(!invoice)throw routeError('Fakturan hittades inte.','INVOICE_NOT_FOUND',404);
        const result=Db.transaction(db,()=>{const posting=Accounting.postEntry(db,{companyId:s.companyId,series:'B',postingDate:invoice.invoiceDate,description:`Leverantörsfaktura ${invoice.supplierInvoiceNumber} ${invoice.supplierName}`,sourceType:'supplier-invoice',sourceId:invoice.id,createdBy:s.userId,lines:invoice.coding});const payment=Payables.preparePayment(db,{companyId:s.companyId,invoiceId:invoice.id,paymentDate:payload.paymentDate,amountOre:invoice.totalOre,account:payload.account||'1930',preparedBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'SUPPLIER_INVOICE_POSTED_AND_PAYMENT_PREPARED',entityType:'supplier-invoice',entityId:invoice.id,details:{accountingEntryId:posting.entry.id,accountingEntryNumber:posting.entry.number,paymentId:payment.id,amountOre:payment.amountOre,paymentDate:payment.paymentDate}});return{posting,payment}});return send(res,201,{payment:result.payment,accountingEntry:result.posting.entry,accountingDuplicate:result.posting.duplicate}),true
      }
