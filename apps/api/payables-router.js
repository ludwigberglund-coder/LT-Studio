...
function createPayablesRouter(options){
  ...
  async function handle(req,res){
    ...
    try{
      ...
      if(url.pathname==='/api/v1/payables/invoices'&&req.method==='POST'){permission(s,'supplier-invoice.register');const payload=await readJson(req,res);if(!payload)return true;const vatTreatment=String(payload.vatTreatment||'').trim();if(vatTreatment!=='se-domestic-full-input-vat')throw routeError('Den här versionen får endast registrera svenska leverantörsfakturor där hela den angivna momsen är avdragsgill. EU, import, omvänd skattskyldighet, momsfritt och begränsad avdragsrätt måste hanteras utanför systemet tills uttryckligt stöd är verifierat.','UNSUPPORTED_SUPPLIER_VAT_TREATMENT',422);if(!Number.isSafeInteger(payload.vatOre)||payload.vatOre<=0)throw routeError('En svensk standardfaktura i detta flöde måste ha ett positivt momsbelopp. Fakturor utan debiterad moms kräver ett annat verifierat momsflöde.','UNSUPPORTED_SUPPLIER_VAT_TREATMENT',422);const invoice=Db.transaction(db,()=>{const value=Payables.createSupplierInvoice(db,{...payload,companyId:s.companyId,registeredBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'SUPPLIER_INVOICE_REGISTERED',entityType:'supplier-invoice',entityId:value.id,details:{supplierId:value.supplierId,supplierInvoiceNumber:value.supplierInvoiceNumber,totalOre:value.totalOre,dueDate:value.dueDate,vatTreatment}});return value});return send(res,201,{invoice,vatTreatment}),true}
