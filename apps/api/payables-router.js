...
function createPayablesRouter(options){
  ...
  async function handle(req,res){
    ...
    try{
      ...
      if(url.pathname==='/api/v1/payables/invoices'&&req.method==='POST'){permission(s,'supplier-invoice.register');const payload=await readJson(req,res);if(!payload)return true;const invoice=Db.transaction(db,()=>{const value=Payables.createSupplierInvoice(db,{...payload,companyId:s.companyId,registeredBy:s.userId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'SUPPLIER_INVOICE_REGISTERED',entityType:'supplier-invoice',entityId:value.id,details:{supplierId:value.supplierId,supplierInvoiceNumber:value.supplierInvoiceNumber,totalOre:value.totalOre,dueDate:value.dueDate}});return value});return send(res,201,{invoice}),true}
      ...
      if(postMatch&&req.method==='POST'){permission(s,'accounting.post');const result=SupplierAccounting.postSupplierInvoice(db,{companyId:s.companyId,invoiceId:postMatch[1],actorId:s.userId});return send(res,200,{...result,message:result.duplicate?`Leverantörsskulden var redan bokförd som ${result.entry.number}.`:`Leverantörsskulden är bokförd som ${result.entry.number}.`}),true}
