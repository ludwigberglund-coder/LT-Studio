...
function createPayablesRouter(options){
  ...
  async function handle(req,res){
    ...
    try{
      ...
      if(url.pathname==='/api/v1/payables/suppliers'&&req.method==='POST'){permission(s,'supplier-invoice.register');const payload=await readJson(req,res);if(!payload)return true;const supplier=Db.transaction(db,()=>{const value=Payables.createSupplier(db,{...payload,companyId:s.companyId});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'SUPPLIER_CREATED',entityType:'supplier',entityId:value.id,details:{supplierNumber:value.supplierNumber,name:value.name}});return value});return send(res,201,{supplier}),true}
      ...
      if(pdfMatch&&req.method==='PUT'){permission(s,'supplier-invoice.register');const bytes=await readPdf(req,res);if(!bytes)return true;const stored=Db.transaction(db,()=>{const value=Payables.storeDocument(db,{companyId:s.companyId,invoiceId:pdfMatch[1],name:documentName(req),mime:'application/pdf',bytes});Db.appendAudit(db,{companyId:s.companyId,userId:s.userId,action:'SUPPLIER_INVOICE_DOCUMENT_STORED',entityType:'supplier-invoice',entityId:pdfMatch[1],details:{sha256:value.sha256,size:value.size}});return value});return send(res,201,{document:stored}),true}
