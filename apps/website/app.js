...
function render(company, site, previewMode) {
  ...
  app.innerHTML = `
    ${previewMode ? '<div class="preview-banner">Lokal förhandsvisning från CMS. Inga ändringar är publicerade på den riktiga webbplatsen.</div>' : ''}
    <header class="site-header"><a class="brand" href="#hem" aria-label="Rollands Saluhall – startsida"><span>${escapeHtml(company.displayName)}</span><small>${escapeHtml(company.address?.city||'Billdal')}</small></a><button class="menu-button" type="button" aria-expanded="false" aria-controls="site-navigation">Meny</button><nav id="site-navigation" class="site-navigation" aria-label="Huvudmeny">${renderNavigation(site.navigation)}</nav><a class="header-action" href="#kontakt">Kontakt</a></header>
    <main id="main">
      <section id="hem" class="hero section-shell"><div class="hero-copy"><p class="eyebrow">${escapeHtml(site.hero.eyebrow)}</p><h1>${escapeHtml(site.hero.title)}</h1><p class="lead">${escapeHtml(site.hero.body)}</p><div class="actions"><a class="button primary" href="${escapeHtml(safeHref(site.hero.primaryCta.href))}">${escapeHtml(site.hero.primaryCta.label)}</a><a class="button secondary" href="${escapeHtml(safeHref(site.hero.secondaryCta.href))}">${escapeHtml(site.hero.secondaryCta.label)}</a></div><div class="highlights">${renderHighlights(site.highlights)}</div></div><div class="hero-visual" aria-label="Illustration av frukt och grönsaker"><div class="visual-card visual-main"><span class="visual-label">Rollands</span><strong>Frukt &amp; grönt</strong><small>${escapeHtml(company.address?.city||'Billdal')}</small></div><div class="produce produce-orange"></div><div class="produce produce-green"></div><div class="produce produce-red"></div><div class="leaf leaf-one"></div><div class="leaf leaf-two"></div><div class="visual-note">Butik · Företag · Service</div></div></section>
    ...
    <footer class="site-footer"><div><strong>${escapeHtml(company.displayName)}</strong><span>${escapeHtml(site.footer.tagline)}</span></div><div class="footer-links"><span>Org.nr ${escapeHtml(company.orgNumber)}</span><a href="./admin/#/content">${escapeHtml(site.footer.adminLabel)}</a></div></footer>`;
...
async function boot() {
  ...
  } catch (error) { app.innerHTML=`<main class="loading-shell error-shell"><p class="eyebrow">Rollands Saluhall</p><h1>Webbplatsen kunde inte laddas</h1><p>${escapeHtml(error.message)}</p></main>`; }
}
boot();
