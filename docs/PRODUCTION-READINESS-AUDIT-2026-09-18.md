...
| P03 | BLOCKER | `accounting_entries`, rader, fakturaunderlag och `audit_events` saknar tillräckliga skydd mot omskrivning/radering i datalagret. `documentForInvoice` läser JSON utan att jämföra lagrad hash. Vanliga API-användare har inte generell SQL-åtkomst, men kodfel, administration eller migrering kan skada historiken. | Öppet. Inför kontrollerad oföränderlighet, verifiering vid läsning och externt skyddat revisionsankare. |
| P04 | BLOCKER | `accounting-admin.correctEntry` skapar motpost men uppdaterar inte tillhörande reskontra eller betalningsstatus. Generell rättelse av en faktura-/betalningspost kan därför skilja 1510/2440 från verklig reskontra. | Öppet. Källanknutna rättelser eller blockering av fel rättelseväg krävs. |
...
| P07 | BLOCKER | Pilotservern svarade 404 för `/shared/vendor/pdf-lib.min.js` och `/content/site.json`. `staticHeaders` använder `object-src 'none'`, medan attestvyn använder inbäddad PDF. API-dokumentens frame-policy måste också passa den skyddade visaren. | Reproducerade HTTP-svar och kodbelägg. Testa rättad PDF/CMS mot **API-servern**, inte bara statiskt demo. |
| P08 | BLOCKER | Portalens `?demo`-växling och demofiler kan nås även via pilotservern. Det är inte i sig en autentiseringsförbikoppling men kan lura användaren att data sparats på servern. Miljöförkontrollen är ett separat kommando, inte en säker startspärr. | Öppet. Explicit miljöläge, inga demofiler/data i pilotbygget och startspärr även bakom lokal HTTPS-proxy. |
...
| P13 | HIGH | Liveness och trafikberedskap måste skiljas åt. En process kan leva samtidigt som databasen är korrupt, oskrivbar eller lagringen nästan full. | **Delvis åtgärdat:** separat `/ready` provar SQLite integrity/foreign keys, skrivväg, privat databasfil och miniminivå för ledigt diskutrymme och returnerar 503 utan råa feldata. `/health` är fortsatt ren liveness. Strukturerad loggtransport, verkligt uptime-larm samt timeout-/disk-full-/DB-avbrottsprov hos driftleverantören återstår. |
