...
| Krypterad extern backup, retention och larm | 🟡 Delvis klar | Krypterad `.sqlite.enc`, lokal retention, restore-drill och monitoreringsgate finns. Den nya R2 EU-offsite-adaptern kan endast skicka verifierad `.enc` + checksumma under immutable SHA-nyckel och kräver remote read-back innan evidens skrivs. Faktisk återkommande offsite-körning, fjärretention/raderingsskydd, larm och restore från den riktiga offsite-kopian återstår. |
