...
| Health/readiness, driftlogg och fungerande larm | 🟡 Delvis klar | DB, skrivbarhet, disk och backup kontrolleras. I staging kräver readiness färsk R2-objektaudit och verifierad restore direkt från R2; extern monitorering bootstrapas via `/api/v1/readiness/core`. Servern har nu dessutom vendor-neutral JSONL-driftloggning med gemensamt `X-Request-Id`, route-klass, status och duration samt redigerad `LOGIN_FAILURE_THRESHOLD`; query, cookies, body, IP, användarnamn, credentials och databasväg loggas inte. Writer-fel får inte krascha requesten. LT Studio har separat read-only `/operator/`-admin med MFA. Faktisk central loggtransport/retention/larm hos hosting-/loggleverantör och verkligt drift-/larmbevis återstår. |
