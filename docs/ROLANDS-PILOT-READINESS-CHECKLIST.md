...
| Moms i faktura och leverantörsbokföring | 🟡 Delvis klar | Kundfakturans svenska normalfall är regelstyrda. PR 103 gör även leverantörsregistreringen fail-closed: endast uttryckligt klassad svensk faktura där hela angivna momsen är avdragsgill får registreras i detta flöde. EU, import, omvänd skattskyldighet, momsfritt, noll moms och begränsad avdragsrätt blockeras tills särskilt stöd är verifierat. Komplett scenario- och deklarationsverifiering återstår. |
...
## Verifierad uppföljning 2026-09-20 – leverantörsmoms fail-closed

PR 103 stoppar den privata leverantörsfakturaregistreringen från att tyst behandla okända momsfall som vanlig svensk ingående moms. API:t kräver `vatTreatment: se-domestic-full-input-vat` och ett positivt angivet momsbelopp. Saknad klassning, EU/import, omvänd skattskyldighet, momsfritt/noll moms och begränsad avdragsrätt svarar `UNSUPPORTED_SUPPLIER_VAT_TREATMENT` innan någon faktura skapas. Audit för en tillåten registrering sparar även vald momsbehandling. Regressionstest visar att blockerade fall lämnar `supplier_invoices` tom och att normal svensk faktura fortfarande går vidare. Detta är en medveten begränsning, inte full momsautomatik. **NO-GO kvarstår.**
