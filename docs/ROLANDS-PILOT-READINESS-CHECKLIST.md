...
| Krypterad extern backup, retention och larm | 🟡 Delvis klar | Krypterad `.sqlite.enc`, lokal retention, restore-drill och monitoreringsgate finns. R2 EU-offsite-adaptern skickar endast verifierad `.enc` + checksumma under immutable SHA-nyckel och kräver full remote read-back innan evidens skrivs. Det finns nu även ett separat fail-closed restore-flöde som hämtar just de verifierade R2-objekten, kontrollerar SHA/storlek/checksumma, dekrypterar isolerat, verifierar SQLite/tenant/journal/privata objekt och raderar arbetskopior innan offsite-restore-evidens skrivs. Faktisk återkommande offsite-körning med riktiga credentials, fjärretention/raderingsskydd och larm återstår. |
...
Det finns nu även en separat kodväg som kan hämta den verifierade backupen direkt från R2 och köra samma integritetskontroller utan att använda den lokala backupkopian. Detta är fortfarande **inte** ett verkligt driftbevis förrän kommandot har körts mot den riktiga privata staging-bucketen med riktiga credentials. **NO-GO kvarstår.**
...
Katastrofåterställningskedjan har fått en separat R2-restore-väg. Kommandot hämtar backupfil och checksumobjekt från den bucket som matchar det senaste offsite-upload-evidenset, streamar till en unik privat arbetskatalog, verifierar SHA-256/storlek/checksumma, dekrypterar en isolerad kopia och kör full SQLite-, foreign-key-, tenant-, journal- och privatobjektskontroll. Arbetskopior raderas innan ett separat offsite-restore-evidens får skrivas. Skyddad readiness kräver detta bevis och accepterar det i högst 30 dagar.
