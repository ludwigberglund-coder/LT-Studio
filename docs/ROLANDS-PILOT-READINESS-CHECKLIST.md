...
| Inloggning, sessionscookie, MFA och CSRF | 🟡 Delvis klar | PR 73 täcker sessionstid/TOTP/medlemskap. PR 123 gör brute-force-räknaren SQLite-baserad så spärren överlever serveromstart. PR 124 verifierar lösenords-/MFA-recovery och lägger atomisk rotation av MFA-master-nyckeln. Verklig recovery-/nyckelrotationsövning i pilotmiljön återstår. |
...
| Lokalt tekniskt backup-/restore-verktyg | 🟡 Delvis klar | PR 64/66/94 verifierar SQLite, journal och dokumentintegritet. PR 119 lägger isolerad restore drill från senaste krypterade backup och kräver färskt evidens i readiness. Verklig restoreövning på pilotserver/offsite-kopia återstår. |
| Krypterad extern backup, retention och larm | 🟡 Delvis klar | PR 116 krypterar offsite-artefakten och verifierar restore. PR 117 lägger dry-run-first retention utifrån godkänd operationsfil. PR 122 kräver färskt externt probe-/larmbevis i readiness. Verklig extern lagring, retentionkörning och larmtest måste fortfarande bevisas i drift. |
...
| Health/readiness, driftlogg och fungerande larm | 🟡 Delvis klar | PR 95 kontrollerar DB, skrivbarhet, disk och backup. PR 119 kräver färskt restore-drill-evidens. PR 122 kräver extern HTTPS-readiness-probe och verifierad larmleverans högst sju dagar gammal. Central logginsamling och verkliga driftbevis återstår. |
...
## Konsoliderad status 2026-09-20 efter PR 117, 119, 122–124

`main` innehåller nu lokal retention med dry-run-first, isolerad restore drill med färskhetskrav i readiness, extern monitor-/larm-evidensgate, persistent login-throttling över omstart samt testad konto-recovery och atomisk MFA-master-key-rotation. Dessa tekniska kontroller minskar blockerarna men ersätter inte verklig offsite-lagring, verkliga larm, recovery-/restoreövning i pilotmiljön eller signerad UAT. **Samlat beslut är fortsatt NO-GO.**
