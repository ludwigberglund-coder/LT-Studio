...
| Arkivering av original och långsiktig läsbarhet | 🟡 Delvis klar | Exakt utfärdad kund-/kredit-PDF och dokumentintegritet är skyddade. PR 200 inventerar och SHA-verifierar alla tre privata filflöden, PR 202/204/208 ger recoverable ledger, idempotent planering och verifierad async copy-worker, och PR 213 ger en fail-closed R2 EU-stagingadapter. Ett read-only R2-auditverktyg kan nu kräva ready-ledger för varje aktuellt privat objekt och läsa tillbaka samtliga stagingkopior för ny storleks-/SHA-256-verifiering med privat evidens utanför Git. Ordinarie runtime läser fortfarande SQLite; verklig R2-körning med staging-credentials, restore från extern objektlagring, retention och produktions-cutover återstår. |
