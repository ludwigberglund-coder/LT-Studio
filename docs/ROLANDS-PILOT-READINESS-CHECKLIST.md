...
Senast granskad: 2026-09-21. Företag: referenskund (identitet lagras privat).
...
| Dubbelklick/idempotens i alla affärsflöden | 🟡 Delvis klar | Kundfaktura/kredit och tidigare kärnflöden är skyddade. PR 212 härdar bank, leverantörsbokföring, betalningsförberedelse/-bekräftelse och lön; PR 217 gör lagermutationer retry-säkra och PR 221 leverantörsändringar/flera beslut retry-säkra. PR #322 gör kundskapande retry-säkert. Den nya idempotensetappen PR #327/#328/#329/#331/#332/#335 gör dessutom identiska retries retry-vänliga för löneimport, leverantörsfakturaregistrering, lagerartikelsskapande, CMS-spara/publicera/återställ, leverantörsskapande samt periodlås/upplåsningsbegäran. För dessa flöden skapas audit endast vid första verkliga mutationen och ändrad payload stoppas fail-closed. Raden hålls 🟡 eftersom återstående mutationer fortfarande måste inventeras, särskilt leverantörsfakturans PDF-/konteringssparning samt beslut/rättelser som idag säkert men medvetet svarar 409 vid retry. |
...
| Fullständig företagsisolering och IDOR | 🟡 Delvis klar | PR 206 spärrar polymorfa dokumentlänkar mot fel företag och PR 209 utökar den metodmedvetna HTTP-IDOR-matrisen. PR 223 kör dessutom kund nummer två genom riktig kundfaktura, PDF och bokföring och kräver 404 från företag A. HTTP-regressioner täcker dokumentarkiv, kund- och leverantörsregister, lager, lön, bank och automationskö. PR #320 utökar dessutom den verkliga HTTP-gränsen för finansobjekt: företag A får 404 när det försöker läsa/ändra företag B:s leverantörsfaktura, frisläppa/bekräfta/rätta B:s betalning, läsa/rätta B:s verifikation eller besluta om B:s periodupplåsning; testet verifierar samtidigt att data och audit förblir oförändrade. Startup-regler klassificerar privata tabeller efter tenant-scope. Nya routes/objekttyper måste fortsatt omfattas av samma blockerande matris. |
...
| PDF-visning i pilotens attest/fakturering | 🟡 Delvis klar | Privat vendor-resurs, iframe och objektbehörighet är testade. PR 91 bevarar dessutom exakt genererade PDF-bytes för utfärdad kund- och kreditfaktura och serverar dem företagsisolerat efter hashkontroll. Full kund-UAT i avsedd drift återstår. |
...
| Lokalt tekniskt backup-/restore-verktyg | 🟡 Delvis klar | SQLite, journal och dokumentintegritet verifieras och krypterad backup kan återställas till isolerad kopia. Restore-kontrollen verifierar dessutom alla tre privata SQLite-filflöden med central objektmetadata/SHA-kontroll och schema-2-evidens. Lokal restore-evidens mäter nu backupens ålder och faktisk verifieringstid; R2-drillen mäter motsvarande ålder för den verifierade offsite-kopian och restore-tiden efter riktig R2-download. Dessa värden ger RPO/RTO-underlag men är inte hårdkodade SLA-gränser. Verklig körning i avsedd stagingmiljö och jämförelse mot beslutad RPO/RTO återstår. |
...
| Health/readiness, driftlogg och fungerande larm | 🟡 Delvis klar | DB, skrivbarhet, disk och backup kontrolleras. I staging kräver readiness färsk R2-objektaudit och verifierad restore direkt från R2; extern monitorering bootstrapas via `/api/v1/readiness/core`. Servern skriver vendor-neutral JSONL med gemensamt `X-Request-Id` och redigerade säkerhetshändelser utan query, cookies, body, rå IP, användarnamn, credentials eller kunddata. Stagingkedjan kräver nu dessutom ett separat privat central-loggbevis: ett riktigt request-id ska ha hittats hos loggleverantören, transport/åtkomst/retention och tre larmklasser ska vara bekräftade, och signoff schema 3 SHA-256-binder evidensfilen. Själva externa loggtjänsten är fortfarande **inte** driftsatt eller verifierad av kodändringen; verklig stagingkonfiguration och faktiskt uppslag/larmbevis återstår. |
...
| Arbetslista och begriplig återkoppling | 🟡 Delvis klar | Dashboarden prioriterar konkreta arbetsköer och visar en tydlig varning när någon datakälla inte kan läsas. PR #306 gör dessutom tillgänglighet explicit per område: misslyckade API-anrop visas som `Ej tillgängligt` i nyckeltal och modulbadges i stället för falska nollor, medan fungerande tomma köer fortsatt visas som riktiga 0-värden. Chromium-test simulerar selektivt 503-fel och verifierar detta. PR #314 låser statussemantiken i fem centrala vyer: leverantörsattest är inte bokföring, automationsgodkännande är inte genomförande, prepared/released/paid hålls isär, bankgranskning är inte bokförd inbetalning och sparat fakturautkast är uttryckligen inte bokfört. Bred verklig kund-UAT av statusorden återstår. |
| Obligatoriska releasekontroller och rollback | 🟡 Delvis klar | Full CI finns och används före merge, men GitHub tvingar ännu inte fram den: `main` är oskyddad och rulesets saknas. BLOCKER #226 måste stängas. Pilot/production-preflight och faktisk serverstart verifierar nu att checkoutens Git-HEAD exakt matchar den godkända `ROLLANDS_RELEASE_COMMIT` och att spårade filer är rena. Verkligt staging-/produktionsbevis återstår. |
...
| referenskunden nio UAT-scenarier mot pilotserver | ❌ Inte klar | Befintliga demo- och kodtester ersätter inte ett signerat pilot-UAT. |
...
Tekniskt ansvarig och redovisningsansvarig ska stänga relevanta BLOCKER-rader med testbevis, exakt releaseversion och datum. Därefter genomför kund-UAT med fiktiva/avidentifierade data i den tänkta driftmiljön. Först efter godkända bevis fattas ett uttryckligt beslut om begränsad pilot, datamängd, användare, varaktighet och stoppkriterier.
...
Detta ändrar **inte** det samlade pilotbeslutet. **NO-GO kvarstår** tills minst verklig staging/restore/monitorering, GitHub branch protection/ruleset enligt BLOCKER #226, nödvändiga driftuppgifter och signerat kund-UAT är verifierade.
...
## Verifierad koduppföljning 2026-09-21 – arbetslista utan falska nollor

PR #306 gör dashboardens datakvalitet synlig. Varje privat delområde har en separat tillgänglighetsstatus. Om exempelvis kundreskontran inte kan läsas visas kundsaldo och relevant modulbadge som `Ej tillgängligt`; samma fel får inte längre se ut som 0 kr eller 0 uppgifter. Samtidigt visas en riktig nolla när ett API-anrop lyckas och kön faktiskt är tom. Dashboarden hämtar nu även faktiska värden för lagerantal, lönekörningar och publicerad CMS-version i privat drift i stället för tysta defaultvärden.

Ett separat Chromium-test körs i ordinarie CI, simulerar selektivt API-fel och verifierar både felstatus och fungerande nollvärden. Nästa avgränsade kontroll är statusordens semantik mellan vyerna.


## Verifierad koduppföljning 2026-09-21 – statussemantik i arbetsflöden

PR #314 lägger blockerande regressioner för användarord som påverkar beslut i portalens kärnflöden. Leverantörsfaktura skiljer `Attesterad` från `Bokför leverantörsskuld`; automationskön visar `Godkänt – ej genomfört` tills execution faktiskt skett; betalningsöversikten skiljer `Förberedd`, `Frisläppt` och `Betald & bokförd`; bankvyn använder `Granskad` innan en inbetalning är bokförd; kundfakturautkast säger uttryckligen att sparat utkast inte är bokfört.

Hela GitHub-CI:n var grön före merge, inklusive alla ordinarie browserflöden. Detta minskar risken att framtida UI-ändringar blandar ihop affärsstatusar. Raden hålls ändå 🟡 tills verklig kund-UAT bekräftar att orden är begripliga i det faktiska arbetssättet.


## Verifierad koduppföljning 2026-09-21 – finance-IDOR över HTTP

PR #320 kör ett gemensamt tvåföretagstest genom den riktiga HTTP-servern. Ett inloggat företag A försöker använda företag B:s leverantörsfaktura, fullt bokförda leverantörsbetalning, manuella verifikation och periodupplåsningsbegäran. Tolv läs-/mutationsförsök måste ge tenant-säker 404. Efter försöken jämför testet faktura och betalning mot före-läget, kräver oförändrat antal bokföringsposter, fortsatt låst period/väntande upplåsningsbegäran och oförändrade auditloggar i båda företagen.

Hela ordinarie GitHub-CI:n var grön före merge. Raden hålls 🟡 eftersom nya framtida objekttyper fortfarande måste föras in i samma blockerande matris och verklig flerföretags-staging/UAT återstår.


## Verifierad koduppföljning 2026-09-21 – kundskapande och retry

PR #322 stänger en konkret dubbelklickslucka i kundregistret utan databasmigration. `POST /api/v1/customers` kräver nu ett klientgenererat requestId. Servern härleder ett deterministiskt kund-ID inom företaget: första försöket skapar kunden och en auditpost, identiskt retry returnerar samma kund med `duplicate:true`, och återanvändning av samma requestId med annan kundpayload stoppas med 409. Kundportalen behåller samma requestId genom ett pågående skapandeförsök och skickar det inte i PUT-redigering.

Hela GitHub-CI:n var grön före merge. Idempotensraden hålls 🟡 eftersom övriga mutationsroutes fortsatt inventeras explicit.


## Verifierad koduppföljning 2026-09-21 – idempotensetapp för skrivflöden

Den här etappen stänger flera konkreta dubbelklicks- och nätverksretry-luckor utan nya databasmigrationer:

- **PR #327 – löneimport:** identisk import återanvänder samma lönekörning och skapar ingen extra `PAYROLL_JOURNAL_IMPORTED`-audit. Samma journalidentitet med ändrat utbetalningsdatum eller ändrade sammanfattningsbelopp stoppas med 409.
- **PR #328 – leverantörsfakturaregistrering:** samma leverantör + normaliserat fakturanummer + identiska fakturauppgifter återanvänder samma faktura. Ändrade datum/belopp med samma identitet stoppas.
- **PR #329 – lagerartikel:** samma SKU och identiska artikeluppgifter återanvänder samma artikel. Ändrat namn/konto med samma SKU stoppas.
- **PR #331 – CMS:** identiska retries av spara utkast, publicera och återställ version returnerar duplicate-success utan ny revision eller extra audit. Stale payload med annan effekt fortsätter stoppas av versionskontrollen.
- **PR #332 – leverantör:** samma leverantörsnummer + identiska masterdata återanvänder leverantören. Ändrade uppgifter med samma nummer stoppas.
- **PR #335 – redovisningsperiod:** samma användares identiska periodlås och identiska väntande upplåsningsbegäran återanvänds. Endast första verkliga låsningen/begäran skapar audit; ändrad upplåsningsorsak fortsätter ge 409.
...
## Verifierad koduppföljning 2026-09-21 – central loggtransport i staging-signoff

Stagingens beviskedja har nu en separat gate för central driftloggning. Ett giltigt bevis kräver ett färskt verkligt `X-Request-Id` som driftansvarig har hittat i den externa loggdestinationen, krypterad transport, begränsad åtkomst, retention som matchar den privata operationsfilen samt konfigurerade larm för säkerhetshändelser, återkommande 5xx och utebliven loggström. Evidensen ligger utanför GitHub och signoff schema 3 binder den med SHA-256.

Detta är endast **kod för att kräva och verifiera beviset**. Ingen extern loggleverantör är konfigurerad av repositoryändringen och inget riktigt request-id har ännu verifierats i drift. **NO-GO kvarstår.**
