...
| Inloggning, sessionscookie, MFA och CSRF | 🟡 Delvis klar | PR 73 verifierar sessionstider och engångs-TOTP. PR 123 gör brute-force-spärren SQLite-baserad så fem fel överlever serveromstart. PR 127 lägger atomisk tvåpersons-kontorecovery och regressionsbevis för auth-nyckelrotation. Verklig recovery-/rotationsövning i drift återstår. |
...
| Krypterad extern backup, retention och larm | 🟡 Delvis klar | PR 116 krypterar offsite-artefakten. PR 117 lägger dry-run-first retention. PR 119 gör isolerad restore drill och kräver färskt evidens i readiness. PR 122 kräver färskt externt readiness- och larmbevis. Verklig offsite-leverantör/kopiering och providerbevis återstår. |
...
| Health/readiness, driftlogg och fungerande larm | 🟡 Delvis klar | PR 95 kontrollerar DB, skrivbarhet, disk och backup. PR 119 kräver färskt restore-drill-bevis och PR 122 kräver färsk extern HTTPS-probe + verifierad larmleverans. Central logginsamling och verkligt provider-/larmevidens återstår. |
| Secrets-hantering och historikskanning | 🟡 Delvis klar | PR 112 skannar full Git-historik. `platform:rotate-auth-key` omkrypterar MFA-hemligheter atomiskt och PR 127 testar lyckad rotation samt fail-closed vid fel gammal nyckel. Secret manager och verklig rotationsövning återstår. |
...
| Betalningsöversikt dag/vecka/månad/kvartal | 🟡 Delvis klar | PR 106 samlar in-/utbetalningar. PR 132 lägger fritext, konto, status/riktning, sortering och Chromium-verifiering mot privat API; totalsummorna räknas efter filtrering. Full pilot-UAT med verklig driftmiljö återstår. |
| Filtrerad Excel-kompatibel export | 🟡 Delvis klar | PR 105 lägger autentiserade Excel-kompatibla CSV-exporter med formelinjektionsskydd. PR 134 låter betalningsöversiktens CSV återanvända exakt samma filtermotor som vyn och verifierar faktisk nedladdning/innehåll i Chromium. Övriga portalvyer behöver fortfarande motsvarande filterkoppling/UAT. |
...
## Konsoliderad status 2026-09-20 – auth, drift, betalningsvy och export

`main` innehåller nu PR 117, 119, 122, 123, 127, 132 och 134. Drift-readiness kräver restore- och monitoreringsbevis, brute-force-spärren överlever omstart, kontorecovery kräver tvåpersonsgodkännande, auth-nyckelrotation har regressionstest och betalningsvyn har detaljfilter/sortering med Chromium-bevis. Betalningsöversiktens CSV återanvänder samma filtermotor och browserprovet verifierar nedladdad fil mot synlig vy. **Samlat beslut är fortsatt NO-GO** tills verklig drift, primärkällor och signerat pilot-UAT är klara.
