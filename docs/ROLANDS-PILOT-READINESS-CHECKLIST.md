...
| Krypterad extern backup, retention och larm | 🟡 Delvis klar | Krypterad `.sqlite.enc`, lokal retention, restore-drill och monitoreringsgate finns. R2 EU-offsite-adaptern skickar endast verifierad `.enc` + checksumma under immutable SHA-nyckel och kräver full remote read-back innan evidens skrivs. Ett separat fail-closed R2-restore-flöde hämtar därefter de faktiska objekten, verifierar SHA/storlek/checksumma, dekrypterar isolerat, verifierar SQLite/tenant/journal/privata objekt och raderar arbetskopior. `staging:evidence:verify` kräver nu även detta offsite-restore-bevis och binder det till exakt samma R2-objektnycklar som upload-beviset. Faktisk körning med riktiga staging-credentials, fjärretention/raderingsskydd och verkligt larm återstår. |
...
Det finns nu en separat kodväg som hämtar den verifierade backupen direkt från R2 och kör samma integritetskontroller utan att använda den lokala backupkopian. Det är fortfarande **inte** ett verkligt driftbevis förrän kommandot har körts mot den riktiga privata staging-bucketen med riktiga credentials. **NO-GO kvarstår.**
...
## Verifierad koduppföljning 2026-09-21 – restore direkt från R2

Katastrofåterställningskedjan har fått en separat R2-restore-väg. Kommandot hämtar backupfil och checksumobjekt från den bucket som matchar offsite-upload-evidenset, streamar till en unik privat arbetskatalog, verifierar SHA-256/storlek/checksumma, dekrypterar en isolerad kopia och kör full SQLite-, foreign-key-, tenant-, journal- och privatobjektskontroll. Arbetskopior raderas innan ett separat offsite-restore-evidens får skrivas. Skyddad readiness kräver detta bevis och accepterar det i högst 30 dagar. Stagingens samlade evidensverifierare kräver dessutom att offsite-restore-beviset gäller exakt samma SHA, storlek och R2-objektnycklar som upload-beviset.

Detta stänger en kodlucka men **inte driftblockeraren**: en riktig stagingserver måste fortfarande genomföra R2-upload och R2-restore med privata credentials samt dokumentera resultatet. **NO-GO kvarstår.**
