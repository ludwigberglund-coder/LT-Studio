...
`npm run pilot:restore:verify` kräver kontrollfil, öppnar källan skrivskyddat utan migrering och kontrollerar SQLite-integritet, främmande nycklar, företagsrelationer, journalbalans och nuvarande modellens nummerserier. Det kopierar till en **ny** destination, kontrollerar kopian och vägrar skriva över en befintlig fil eller angiven produktionsdatabas. En misslyckad kopiekontroll tar endast bort den just skapade destinationsfilen.
...
Fortsätt bara vid exitkod 0 och `verified:true`. Det betyder att de implementerade tekniska kontrollerna passerat, inte att alla bokföringsregler är granskade.
...
`test/pilot-deployment.test.js` kör verkliga backup-/restore-kommandon mot temporära SQLite-filer. Ett nytt fyllt scenario bevarar tre konteringsrader inklusive moms, en audit-händelse och exakt binärt dokumentinnehåll. Sex negativa scenarier nekar saknad/fel kontrollfil, föräldralösa relationer, halv journal, fel nummerserie och företagssammanblandning. Dokumentfixturen testar bytebevarande, inte visuellt giltig fakturadesign.

Testerna bevisar inte extern lagring, krypteringsdrift, myndighetsgodkännande, fullständig dokumenthashverifiering, katastrofåterställning eller avstämning av verkliga ingående balanser.
