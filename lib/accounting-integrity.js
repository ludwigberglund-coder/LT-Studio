...
function validateJournalIntegrity(store) {
  ...
  if (firstSealed < 0) {
    ...
    return {ok: errors.length === 0, errors, warnings, sealed: 0, pending: journal.length};
  }

  for (let index = firstSealed; index < journal.length; index += 1) {
    if (!hasJournalSeal(journal[index])) {
      ...
      return {ok: false, errors, warnings, sealed: journal.length - firstSealed, pending: firstSealed};
    }
  }

  for (let index = journal.length - 1; index >= firstSealed; index -= 1) {
    const entry = journal[index];
    const integrity = entry.integrity || {};
    const expectedPrevious = index === journal.length - 1 ? GENESIS_HASH : journal[index + 1].integrity.hash;
  ...
  const pending = firstSealed;
  const sealed = journal.length - pending;
  if (pending) warnings.push(`${pending} nya verifikationer väntar på integritetsförsegling.`);
