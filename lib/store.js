...
const AccountingIntegrity = require('./accounting-integrity.js');
...
function validateStore(store) {
  ...
  const accountingIntegrity = AccountingIntegrity.validateAccountingIntegrity(store);
  errors.push(...accountingIntegrity.errors);
  warnings.push(...accountingIntegrity.warnings);
...
function atomicWriteJson(file, data) {
  try {
    AccountingIntegrity.sealAccountingIntegrity(data);
  } catch (error) {
    if (error?.code === 'ACCOUNTING_INTEGRITY_ERROR') {
      const report = {
        ok: false,
        errors: error.report?.errors || [error.message],
        warnings: error.report?.warnings || [],
        summary: error.report?.summary || {}
      };
      throw new IntegrityError(`Datalagret kunde inte sparas: ${report.errors[0]}`, report);
    }
    throw error;
  }
...
module.exports = {
  ...
  stripKnownDemoData,
  createBackup,
  validateAccountingIntegrity: AccountingIntegrity.validateAccountingIntegrity,
  sealAccountingIntegrity: AccountingIntegrity.sealAccountingIntegrity
};
