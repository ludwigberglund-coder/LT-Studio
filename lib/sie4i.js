...
function accountName(code, inlineName, options = {}) {
  const configured = cleanText(options.accountNames?.[code] || '');
  return configured || AccountPlan.byCode?.[code]?.name || inlineName || `Konto ${code}`;
}

function amountOre(row) {
  const hasOre = row?.debitOre !== undefined || row?.creditOre !== undefined;
  const hasLegacyKronor = row?.debit !== undefined || row?.credit !== undefined;
  if (hasOre && hasLegacyKronor) throw new Error('SIE-raden blandar ören och äldre kronbelopp och kan inte exporteras säkert.');
  ...
  if (hasOre) {
    ...
    if (!Number.isSafeInteger(debitOre) || !Number.isSafeInteger(creditOre) || debitOre < 0 || creditOre < 0) {
      throw new Error('SIE-export kräver bokföringsbelopp som säkra heltalsören och utan negativa radbelopp.');
    }
    if ((debitOre > 0) === (creditOre > 0)) throw new Error('Varje SIE-transaktion måste ha exakt ett debet- eller kreditbelopp.');
    return BigInt(debitOre) - BigInt(creditOre);
  }

  const debit = Number(row?.debit ?? 0);
  const credit = Number(row?.credit ?? 0);
  if (!Number.isSafeInteger(debit) || !Number.isSafeInteger(credit) || debit < 0 || credit < 0) {
    throw new Error('Äldre SIE-underlag kräver säkra heltalsbelopp i kronor och utan negativa radbelopp.');
  }
  if ((debit > 0) === (credit > 0)) throw new Error('Varje SIE-transaktion måste ha exakt ett debet- eller kreditbelopp.');
  return (BigInt(debit) - BigInt(credit)) * 100n;
}

function formatAmountOre(value) {
...
function assertBalanced(entry) {
  ...
  const total = entry.rows.reduce((sum, row) => sum + amountOre(row), 0n);
  if (total !== 0n) throw new Error(`Verifikation ${entry.number || entry.id || ''} balanserar inte och kan därför inte SIE-exporteras.`);
}
...
function usedAccounts(journal, options = {}) {
  const accounts = new Map();
  for (const entry of journal) {
    for (const row of entry.rows || []) {
      const parsed = parseAccount(row.account);
      if (!accounts.has(parsed.code)) accounts.set(parsed.code, accountName(parsed.code, parsed.inlineName, options));
    }
  }
...
function buildSie4iText(store, options = {}) {
  ...
  const companyType = cleanText(options.companyType || business.companyType || '').toUpperCase();
  if (companyType) {
    if (!/^(?:AB|E|HB|KB|EK|KHF|BRF|BF|SF|I|S|X)$/.test(companyType)) throw new Error('Ogiltig företagsform för SIE-export.');
  ...
  const requestedYear = cleanText(options.fiscalYear || '');
  if (requestedYear && !/^\d{4}$/.test(requestedYear)) throw new Error('Räkenskapsår för SIE-export måste anges med fyra siffror.');
  if (requestedYear && dates.some(date => !date.startsWith(requestedYear))) throw new Error('SIE-underlaget innehåller verifikationer utanför valt räkenskapsår.');
  const firstYear = requestedYear || (dates[0] ? dates[0].slice(0, 4) : '');
  const sameYear = Boolean(firstYear) && dates.every(date => date.startsWith(firstYear));
  if (sameYear || requestedYear) lines.push(`#RAR 0 ${firstYear}0101 ${firstYear}1231`);

  for (const [code, name] of usedAccounts(journal, options)) lines.push(`#KONTO ${code} ${quoted(name)}`);
  ...
  for (const entry of journal) {
    ...
    for (const row of entry.rows) {
      ...
      lines.push(`#TRANS ${code} {} ${formatAmountOre(amountOre(row))} ""`);
    }
    lines.push('}');
  }
