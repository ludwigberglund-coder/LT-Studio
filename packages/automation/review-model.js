...
function reviewError(message,code,statusCode=422){const e=new Error(message);e.code=code;e.statusCode=statusCode;return e}
...
function validateEditedSuggestion(proposal,input){
  if(!proposal||!['manual-review','ready-for-approval'].includes(proposal.status))throw reviewError('Endast öppna förslag kan ändras.','INVALID_PROPOSAL_STATUS',409);
  ...
  if(Array.isArray(edited.accountingLines)){
    if(!edited.accountingLines.length||edited.accountingLines.length>20)throw reviewError('Konteringen måste innehålla 1–20 rader.','INVALID_ACCOUNTING_LINES');
    const lines=edited.accountingLines.map((row,index)=>{const account=text(row?.account);if(!/^\d{4}$/.test(account))throw reviewError(`Konteringsrad ${index+1} har ogiltigt konto.`,'INVALID_ACCOUNT');const debitOre=Number(row?.debitOre||0),creditOre=Number(row?.creditOre||0);if(!Number.isSafeInteger(debitOre)||!Number.isSafeInteger(creditOre)||debitOre<0||creditOre<0||(debitOre&&creditOre))throw reviewError(`Konteringsrad ${index+1} har ogiltigt belopp.`,'INVALID_ACCOUNTING_AMOUNT');return{account,debitOre,creditOre,text:text(row?.text||row?.label).slice(0,240)}});
    const debit=lines.reduce((s,r)=>s+r.debitOre,0),credit=lines.reduce((s,r)=>s+r.creditOre,0);if(debit!==credit||debit<=0)throw reviewError('Den föreslagna konteringen måste balansera i debet och kredit.','UNBALANCED_SUGGESTION');current.accountingLines=lines;
  }
  if(proposal.type==='bank-payment-match'&&Object.hasOwn(edited,'invoiceId')){const invoiceId=text(edited.invoiceId);if(!invoiceId)throw reviewError('En kundfaktura måste väljas.','MISSING_TARGET_INVOICE');current.invoiceId=invoiceId;if(Object.hasOwn(edited,'invoiceNumber'))current.invoiceNumber=text(edited.invoiceNumber)}
